...
T W Sederberg, T Nishita*, CURVE CURVE INTERSECTION
...
Engineering Computer Graphics Lab, Brigham Young University Provo, UT 84602, USA *Department of Electrical Engineering, Fukuyama University

## Abstract

Define a fat line as the region between two parallel lines. The curve intersection algorithm described here begins by computing a fat line which bounds one of the two Bezier curves. Similar bounds have been suggested in References 5, 6 and 7. Denote by I the line Po-Pr. A fat line is chosen parallel to L, as shown in Figure 1. If I is defined in its normalized implicit equation a x+b y+c=0 \quad\left(a^{2}+b^{2}=1\right)
...
$$
\min \left\{0, d_{1}, d_{2}\right\} 3 t(1-t) \leqslant d(t) \leqslant \max \left\{0, d_{1}, d_{2}\right\} 3 t(1-t)
$$

$$
\begin{aligned} & d_{\min }=\min \left\{0, \frac{d_{1}}{2}\right\} \\ & d_{\max }=\max \left\{0, \frac{d_{1}}{2}\right\} \end{aligned}
$$

$$
\begin{aligned} & d_{\min }=\min \left\{0, d\left(t_{1}\right)\right\} \\ & d_{\max }=\max \left\{0, d\left(t_{1}\right)\right\} \end{aligned}
$$

![Figure 2. Fat line for a polynomial quadratic curve](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-2-p002.png)

*Figure 2. Fat line for a polynomial quadratic curve: Figure 2. Fat line for a polynomial quadratic curve*

$$
\begin{aligned} & d_{\min }=\min \left\{d\left(t_{1}\right), d\left(t_{2}\right)\right\} \\ & d_{\max }=\max \left\{d\left(t_{1}\right), d\left(t_{2}\right)\right\} \end{aligned}
$$

$$
\begin{aligned} & d_{\min }=\frac{3}{4} \min \left\{0, d_{1}, d_{2}\right\} \\ & d_{\max }=\frac{3}{4} \max \left\{0, d_{1}, d_{2}\right\} \end{aligned}
$$

$$
d(t)=3 t(1-t)\left[(1-t) d_{1}+t d_{2}\right]
$$

$$
3 t(1-t)^{2} d_{1} \leqslant d(t) \leqslant 3 t^{2}(1-t) d_{2}
$$

$$
t_{1}=\frac{d_{1}}{2 d_{1}-d_{2}+\sqrt{d_{1}^{2}-d_{1} d_{2}+d_{2}^{2}}}
$$

$$
\begin{aligned} d_{\min } & =\frac{4}{9} \min \left\{0, d_{1}, d_{2}\right\} \\ d_{\max } & =\frac{4}{9} \max \left\{0, d_{1}, d_{2}\right\} \end{aligned}
$$

$$
\begin{aligned} & t_{1}=\frac{2 d_{1}-d_{2}+\sqrt{d_{1}^{2}+d_{2}^{2}-d_{1} d_{2}}}{3\left(d_{1}-d_{2}\right)} \\ & t_{2}=\frac{2 d_{1}-d_{2}-\sqrt{d_{1}^{2}+d_{2}^{2}-d_{1} d_{2}}}{3\left(d_{1}-d_{2}\right)} \end{aligned}
$$

![Figure 3. Fat lines for polynomial cubic curves](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-3-p002.png)

*Figure 3. Fat lines for polynomial cubic curves: Figure 3. Fat lines for polynomial cubic curves*

![Figure 4. Bézier curve/tat line intersection](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-4-p003.png)
...
$$
\mathbf{P}(t)=\sum_{i=0}^{n} \mathbf{P}_{i} B_{i}^{n}(t)
$$
...
$$
\mathbf{D}(t)=(t, d(t))=\sum_{i=0}^{n} \mathbf{D}_{i} B_{i}^{n}(t)
$$

Since \(\sum_{i=0}^{n}(i / n) B_{i}^{n}(t) \equiv t[(1-t)+t]^{n} \equiv t\), Values of t for which \(P(t)\) lies outside of L correspond to values of t for which \(D(t)\) lies above d = dmax or below d = dmin. Parameter ranges of t can be identified for which \(P(t)\) is guaranteed to lie outside of L by identifying ranges of t for which the convex hull of Dit) lies above d = dmax Or below d = dmin· In this example, it is certain that \(P(t)\) lies outside of L for parameter values t < 0.25 and for t > 0.75. Bézier clipping is completed by subdividing \(\mathbf{P}\) twice using the de Casteljau algorithm \({ }^{8}\), such that portions of \(\mathbf{P}\) over parameter values \(t<0.25\) and \(t>0.75\) are removed.

![Figure 5. Non-parametric Bézier curve](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-5-p003.png)

*Figure 5. Non-parametric Bézier curve: Figure 5. Non-parametric Bézier curve*

$$
a x+b y+c=0 \quad\left(a^{2}+b^{2}=1\right),
$$

![Figure 71. From Figure 6, it is concluded that it is safe](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-71-p003.png)

*Figure 71. From Figure 6, it is concluded that it is safe: Figure 71. From Figure 6, it is concluded that it is safe*

$$
d(t)=\sum_{i=0}^{n} d_{i} B_{i}^{n}(t), \quad d_{i}=a x_{i}+b y_{i}+c
$$

After three Bézier clips on each curve, the intersection is computed to within six digits of accuracy (see Table Clipping to other fat lines The fat line defined above provides a nearly optimal Bézier clip in many cases. However, it is clear that any pair of parallel lines which bound the curve can serve as a fat line. In many cases, a fat line perpendicular to the line \(\mathbf{P}_{0}-\mathbf{P}_{n}\) provides a larger Bézier clip than does the fat line parallel to the line \(\mathbf{P}_{0}-\mathbf{P}_{n}\). Figure 8 shows

![Figure 6. After the first Bézier clip](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-6-p004.png)

*Figure 6. After the first Bézier clip: Figure 6. After the first Bézier clip*

![Figure 7. Distance from Q(u) to I](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-7-p004.png)
...
![Figure 8. Alternative fat lines](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-8-p004.png)

*Figure 8. Alternative fat lines: Figure 8. Alternative fat lines Table 1. Parameter ranges for P(t) and Q (u)*

![Figure 9. Two intersections](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-9-p004.png)

*Figure 9. Two intersections: Figure 9. Two intersections*

![Figure 10. Two intersections after a split](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-10-p005.png)
...
$$
\mathbf{P}(t)=\frac{\boldsymbol{\Sigma}_{i=0}^{n} w_{i} \mathbf{P}_{i} B_{i}^{n}(t)}{\boldsymbol{\Sigma}_{i=0}^{n} w_{i} B_{i}^{n}(t)}
$$

with control point coordinates \(\mathbf{P}_{i}=\left(\begin{array}{ll}x_{i} & y_{i}\end{array}\right)\) and corresponding non-negative weights \(w_{i}\), the Bézier clip computation is modified as follows. Substituting equation (20) into equation (17) and clearing the denominator yields:

However, unlike the nonrational case, the intersection of \(\mathbf{P}(t)\) with a fat line cannot be represented as \(\left\{(x, y)=\mathbf{P}(t) \mid d_{\text {min }} \leqslant d(t) \leqslant\right.\) \(\left.d_{\text {max }}\right\}\). Instead, \(\mathbf{P}\) must be clipped independently against each of the two lines bounding the fat line. Thus, ranges of \(t\) are identified for which
...
![Figure 11. Collinear normal line](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-11-p005.png)

*Figure 11. Collinear normal line: Figure 11. Collinear normal line*

![Figure 12. Tangent intersection](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-12-p006.png)

*Figure 12. Tangent intersection: Figure 12. Tangent intersection*

$$
\left[\begin{array}{ll} x_{1}-x_{0} & x_{n-1}-x_{n} \\ y_{0}-y_{1} & y_{n}-y_{n-1} \end{array}\right]\left\{\begin{array}{l} c_{0} \\ c_{1} \end{array}\right\}=\left\{\begin{array}{l} y_{n}-y_{0} \\ x_{n}-x_{0} \end{array}\right\}
$$
...
![Figure 13. Focus example 1](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-13-p006.png)

*Figure 13. Focus example 1: Figure 13. Focus example 1*

![Figure 14. Focus example 2](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-14-p007.png)

*Figure 14. Focus example 2: Figure 14. Focus example 2*

![Figure 15. Focus example 3](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-15-p007.png)
...
![Figure 16. Curve and hodograph](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-16-p007.png)

*Figure 16. Curve and hodograph: Figure 16. curve and hodograph*

![Figure 17. Tangent vector](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-17-p007.png)
...
![Figure 18. Rotated hodograph, N(t)](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-18-p008.png)

*Figure 18. Rotated hodograph, N(t): Figure 18. Rotated hodograph, N(t)*

![Figure 19. Curve of Figure 15 (left) split into two segments](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-19-p008.png)

*Figure 19. Curve of Figure 15 (left) split into two segments: Figure 19. curve of Figure 15 (left) split into two segments*

$$
d_{i}=\sum_{\substack{i+k=i \\ j \in\{0, \ldots, n\} \\ k \in\{0, \ldots, n-1\}}} \frac{\binom{n}{j}\binom{n-1}{k}}{\binom{2 n-1}{i}} n\left(\mathbf{P}_{k+1}-\mathbf{P}_{k}\right) \cdot\left(\mathbf{P}_{j}-\mathbf{F}\right)
$$

$$
d(t)=\mathbf{P}^{\prime}(t) \cdot(\mathbf{P}(t)-\mathbf{F})=0
$$

![Figure 20. Curve of Figure 15 split into four segments](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-20-p008.png)

*Figure 20. Curve of Figure 15 split into four segments: Figure 20. curve of Figure 15 split into four segments*

![Figure 23. P. Q and Fa](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-23-p009.png)

*Figure 23. P. Q and Fa: Figure 23. P. Q and Fa*

![Figure 21. Normal to P(t) passing through F](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-21-p009.png)

*Figure 21. Normal to P(t) passing through F: Figure 21. Normal to P(t) passing through F*

$$
d_{i j}=\sum_{\substack{k,+l=i \\ 1 \in\{0, \ldots, n\} \\ k \in\{0, \ldots, n-1\}}} \frac{\binom{n}{j}\binom{n-1}{k}}{\binom{2 n-1}{j}} n\left(\mathbf{P}_{k+1}-\mathbf{P}_{k}\right) \cdot\left(\mathbf{P}_{l}-\mathbf{F}_{j}\right)
$$

![Figure 24. Top view of D(t, u) patch](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-24-p009.png)

*Figure 24. Top view of D(t, u) patch: Figure 24. Top view of D(t, u) patch*

$$
d(t, u)=\sum_{i=0}^{2 n-1} \sum_{j=0}^{m} B_{i}^{2 n-1}(t) B_{j}^{m}(u) d_{i j}
$$

![Figure 22. Non-parametric Bézier curve](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-22-p009.png)

*Figure 22. Non-parametric Bézier curve: Figure 22. Non-parametric Bézier curve*

$$
D(t, u)=\left(\mathbf{P}(t)-\mathbf{F}_{\mathbf{Q}}(u)\right) \cdot \frac{\mathbf{P}^{\prime}(t)}{n} \neq 0,0 \leqslant u \leqslant 1
$$

\(D(t, u)\) can be expressed as a tensor product polynomial in Bernstein form in \(t\) and \(u\) :

![Figure 27. Q after first clipping, and its focus](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-27-p010.png)

*Figure 27. Q after first clipping, and its focus: Figure 27. Q after first clipping, and its focus*

$$
\begin{aligned} \mathbf{D}(t, u) & =\sum_{i=0}^{2 n-1} \sum_{j=0}^{m} B_{i}^{2 n-1}(t) B_{j}^{m}(u) D_{i j} \\ & =(t, u, d(t, u)) \end{aligned}
$$

![Figure 25. Side view of D(t, u) patch](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-25-p010.png)

*Figure 25. Side view of D(t, u) patch: Figure 25. Side view of D(t, u) patch*

![Figure 26. P after first clipping, and its focus](/Users/evanthayer/Projects/paperx/docs/1990_curve_intersection_using_bezier_clipping/figures/figure-26-p010.png)
...
- 1 Lane, | M and Riesenfeld, R 'A theoretical development for the computer generation and display of piecewise polynomial surfaces' IEEE Trans. RAMI Vol 2 (1980) pp 35-46
