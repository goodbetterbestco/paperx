...
A lifting of an immersion \(i\) is an embedding \(e: M \rightarrow \mathbb{R}^{3}\) that projects to \(i\) : that is, for all \(p \in M, i(p)=\) \(\pi_{\mathrm{z}}(e(M))\), where \(\pi_{\mathrm{z}}((x, y, z))=(x, y)\). Necessarily, \(e\) must describe a generalized terrain, for otherwise its projection would not be a local homeomorphism.
...
![Figure 2. A generalized terrain, as viewed from above. Figure 3. This curve is the boundary of a unique immersed surface, topologically equivalent to a punctured torus.](/Users/evanthayer/Projects/paperx/docs/2009_self_overlapping_curves_revisited/figures/figure-2-p004.png)
...
![Figure 5. Bennequin’s curve bounding ﬁve different immersed disks.](/Users/evanthayer/Projects/paperx/docs/2009_self_overlapping_curves_revisited/figures/figure-5-p004.png)
...
Theorem 2. Given a cased oriented curve, we can determine whether the cased curve represents the boundary of a generalized terrain and find a terrain having it as a boundary, in \(O\left(\min \left(n_{k}, n+k^{3}\right)\right)\) time, where \(n\) is the number of self-crossings of the curve and \(k\) is the number of its components.
...
![Figure 6. Reduction from P LANAR 3- COLORABILITY to immersibility or embeddability of a curve.](/Users/evanthayer/Projects/paperx/docs/2009_self_overlapping_curves_revisited/figures/figure-6-p007.png)
...
We transform a planar graph G =(V, E) into a family CG of | V | + 3 simple closed curves. | V | of these curves, which we call vertex-curves, are oriented counterclockwise and represent the vertices of G. In any immersion or embedding having CG as boundary, vertex-curves must be hole boundaries due to their orientation. We place these curves in such a way that every pair of vertex-curves are either disjoint or have two points in common, and such that a pair of vertex-curves intersect if and only if the corresponding vertices of the graph are adjacent. The remaining three curves in CG, denoted \(c_{1}\), \(c_{2}\) and \(c_{3}\), are oriented clockwise and surround all the vertex-curves. These three curves are disk-boundaries. See Figure 6(a),(b) for an example.

Lemma 1. If G has a three-coloring then there exists an immersed surface in the plane of which CG is a boundary, having the topology of three disks with holes.

Proof. Form a disk in the plane for each of \(c_{1}\), \(c_{2}\), and \(c_{3}\). For each vertex-curve cv, corresponding to a vertex v with color i, form a hole with boundary cv in the disk bounded by ci. Due to the coloring of G, no two holes in the same disk overlap, so this forms a valid immersion of three punctured disks with boundaries \(c_{1}\), \(c_{2}\) and \(c_{3}\), together with the holes formed as above. intersectionsq unionsq Corollary 1. If G has a three-coloring then CG can be embedded in space as the boundary of a generalized terrain.

Proof. Lift the three punctured disks of Lemma 1 to distinct heights in \(R_{3}\). intersectionsq unionsq Next we prove that given a collection of surfaces S immersed in the plane so that the boundaries of S match the reduction CG of a graph G we can define \(a_{3}\)-coloring of G from S.

To do so, define a relation ∼ between hole-boundaries of S : \(b_{1}\) ∼ \(b_{2}\) if and only if there exists a curve in S that starts on \(b_{1}\), ends on \(b_{2}\), and does not pass through any points where two holes overlap.
...
To prove a similar hardness result for unoriented curves, we simply replace each vertex-curve in the reduction by a curve that can only be embedded with one orientation, namely a curve with two self-intersections as depicted in Figure 7. If the self-crossing parts of these curves do not cross the other curves, they can only be oriented in such a way that the two outer lobes \(c_{1}\) and \(c_{2}\) of the curve act like hole boundaries in whatever surface they bound, i.e. for any immersed surface \(i\) the neighborhoods of \(c_{1}\) and \(c_{2}\) are mapped outside of \(i\left(c_{1}\right)\) and \(i\left(c_{2}\right)\) correspondingly. Hence, Corollary 3. It is NP-complete to determine whether a curve can be seen as a boundary of an immersed surface in the plane or an embedded surface in space.

![Figure 7. A curve that can only be embedded with one orientation.](/Users/evanthayer/Projects/paperx/docs/2009_self_overlapping_curves_revisited/figures/figure-7-p009.png)
...
![Figure 8. Uncrossing the dashed region bounded by two thick segments of curves and two indicated crossing points.](/Users/evanthayer/Projects/paperx/docs/2009_self_overlapping_curves_revisited/figures/figure-8-p009.png)
...
![Figure 9. Reduction from A CYCLIC P ARTITION to liftability of an immersed surface.](/Users/evanthayer/Projects/paperx/docs/2009_self_overlapping_curves_revisited/figures/figure-9-p010.png)
...
![Figure 10. Gadgets for the reduction from N OT - ALL - EQUAL -3 SAT to A CYCLIC P ARTITION .](/Users/evanthayer/Projects/paperx/docs/2009_self_overlapping_curves_revisited/figures/figure-10-p011.png)
...
![Figure 11. An immersed surface with 2 n / 2 combinatorially differ- ent spatial embeddings.](/Users/evanthayer/Projects/paperx/docs/2009_self_overlapping_curves_revisited/figures/figure-11-p012.png)
...
![Figure 12. The graph G formed from the crossings of an immersed surface. The curve shown in black is the boundary of a unique immersed surface; the corresponding graph consists of two 4-cycles and one 6-cycle (colored red, green, and blue).](/Users/evanthayer/Projects/paperx/docs/2009_self_overlapping_curves_revisited/figures/figure-12-p012.png)
