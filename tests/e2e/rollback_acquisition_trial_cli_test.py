...
class RollbackAcquisitionTrialCliE2ETest(unittest.TestCase):
    def test_cli_rolls_back_promoted_trial_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (backup_dir / "acquisition-route.json").write_text(
                json.dumps({"primary_route": "born_digital_scholarly"}, indent=2) + "\n",
                encoding="utf-8",
            )
            (backup_dir / "source-scorecard.json").write_text(
                json.dumps({"recommended_primary_layout_provider": "docling"}, indent=2) + "\n",
                encoding="utf-8",
            )
            (backup_dir / "layout.json").write_text(
                json.dumps({"engine": "docling", "blocks": [{"id": "d1"}]}, indent=2) + "\n",
                encoding="utf-8",
            )
            (backup_dir / "math.json").write_text(
                json.dumps({"engine": "docling", "entries": [{"id": "doc-eq-1"}]}, indent=2) + "\n",
                encoding="utf-8",
            )
            (backup_dir / "acquisition-execution.json").write_text(
                json.dumps({"executed": {"selected_layout_provider": "docling"}}, indent=2) + "\n",
                encoding="utf-8",
            )
            (trial_dir / "promotion.json").write_text(
                json.dumps(
                    {
                        "label": "trial-mathpix",
                        "previous_live_paths": {
            ...
            completed = subprocess.run(
                [
                    sys.executable,
                    "-m",
                    "pipeline.cli.rollback_acquisition_trial",
