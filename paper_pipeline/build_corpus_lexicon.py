...
from paper_pipeline.lexicon import DOCS_DIR, LEXICON_PATH
...
def _collect_bibliography_sources() -> tuple[Counter[str], dict[str, set[str]], Counter[str], dict[str, set[str]]]:
    ...
    bibliography_path = DOCS_DIR / "bibliography.json"
    if not bibliography_path.exists():
        return token_counts, token_papers, acronym_counts, acronym_papers
...
def _collect_canonical_sources(
    ...
    for canonical_path in DOCS_DIR.glob("*/canonical.json"):
...
def _build_lexicon() -> dict[str, Any]:
    token_counts, token_papers, acronym_counts, acronym_papers = _collect_bibliography_sources()
    (
        token_counts,
        token_papers,
        acronym_counts,
        ...
        author_counts,
        author_papers,
    ) = _collect_canonical_sources(token_counts, token_papers, acronym_counts, acronym_papers)
    ...
    if bibliography_path.exists():
        bibliography_entries = len(json.loads(bibliography_path.read_text(encoding="utf-8")).get("entries", []))

    return {
        "schema_version": 1,
        "sources": {
            "bibliography_entries": bibliography_entries,
            "canonical_papers": sum(1 for _ in DOCS_DIR.glob("*/canonical.json")),
        },
        "terms": terms,
        "authors": authors,
        "venues": venues,
