...
from paper_pipeline.corpus_layout import canonical_sources_dir
from paper_pipeline.math_review_policy import review_for_math_entry
from paper_pipeline.types import LayoutBlock, default_formula_conversion, default_review


def _paper_sources_dir(paper_id: str) -> Path:
...
def external_layout_path(paper_id: str) -> Path:
    return _paper_sources_dir(paper_id) / "layout.json"


def external_math_path(paper_id: str) -> Path:
    return _paper_sources_dir(paper_id) / "math.json"


def load_external_layout(paper_id: str) -> dict[str, Any] | None:
    return _load_layout_payload(external_layout_path(paper_id), default_engine="external_layout")


def load_mathpix_layout(paper_id: str) -> dict[str, Any] | None:
    return _load_layout_payload(_paper_sources_dir(paper_id) / "mathpix-layout.json", default_engine="mathpix_layout")
...
def load_external_math(paper_id: str) -> dict[str, Any] | None:
    path = external_math_path(paper_id)
    if not path.exists():
        return None
    payload = json.loads(path.read_text(encoding="utf-8"))
