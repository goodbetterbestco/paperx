...
from paper_pipeline.runtime_paths import ensure_repo_tmp_dir
...
def audit_document(path: Path) -> dict[str, Any]:
    ...
    bibliography_entry = paper_metadata(str(document.get("paper_id", "")))
    ...
    return {
        ...
        "year": bibliography_entry.get("year") if isinstance(bibliography_entry, dict) else None,
...
def audit_missing_canonical(paper_dir: Path) -> dict[str, Any]:
    bibliography_entry = paper_metadata(paper_dir.name)
    title = paper_dir.name.replace("_", " ")
    if isinstance(bibliography_entry, dict) and bibliography_entry.get("title"):
        title = str(bibliography_entry["title"])
