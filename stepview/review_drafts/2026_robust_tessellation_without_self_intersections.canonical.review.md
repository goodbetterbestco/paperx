...
![Figure 1](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-1-p002.png)
...
![Figure 2](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-2-p002.png)
...
Direct methods construct meshes entirely in \(\mathbb{R}^{3}\), without ref erence to the underlying surface parametrization. Classical ex amples include Delaunay triangulation and its refinement vari ants (Cheng et al., 2013; Ruppert, 1995), advancing-front tech niques (Cuillière, 1998; Schöberl, 1997), and octree-based subdi vision (Shostko et al., 1999; Yerry \& Shephard, 1984), all of which excel in automation and robustness. However, these approaches often require excessive refinement to conform to intricate trim ming curves and may produce poorly shaped elements in regions of high curvature.

Parametric methods (Cripps \& Parwana, 2011; Sheng \& Hirsch, 1992) leverage the bijective mapping \(\mathrm{S}: \Omega \subset \mathbb{R}^{2} \rightarrow \mathbb{R}^{3}\) of NURBS or spline patches. A high-quality two-dimensional mesh-often

![Figure 3](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-3-p003.png)
...
![Figure 4](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-4-p003.png)
...
$$
\begin{array}{ll} \mathcal{S}=\left\{\mathrm{s}_{0}, \mathrm{~s}_{1}, \ldots, \mathrm{~s}_{n}\right\}, & 0=\mathrm{s}_{0}<\mathrm{s}_{1}<\cdots<\mathrm{s}_{n}=1 \\ \mathcal{T}=\left\{\mathrm{t}_{0}, \mathrm{t}_{1}, \ldots, \mathrm{t}_{m}\right\}, & 0=\mathrm{t}_{0}<\mathrm{t}_{1}<\cdots<\mathrm{t}_{m}=1 \end{array}
$$

Definition 1 (critical point). Let \(\mathrm{C}_{1}(\mathrm{~s}):[0,1] \rightarrow \mathbb{R}^{2}\) and \(\mathrm{C}_{2}(\mathrm{t})\) : \([0,1] \rightarrow \mathbb{R}^{2}\) be two NURBS curves satisfying \(C_{1}(s) \neq C_{2}(t)\) for all \(s, t \in[0,1]\). A pair of partitions

![Figure 5](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-5-p004.png)
...
Lemma 1 (Non-intersection via Critical Points). Let \(C_{1}\) and \(C_{2}\) be two non-intersecting NURBS curves. If \(\mathcal{S}\) and \(\mathcal{T}\) are critical points for ( \(C_{1}, C_{2}\) ) as per definition 1, then the polylines obtained by con necting consecutive points in \(\mathcal{S}\) and \(\mathcal{T}\) are guaranteed to be non intersecting. Furthermore, this non-intersection property is pre served under any refinement of the critical points.
...
We require that each curve involved satisfy the Angular Mono tonicity condition: that is, any straight line through the endpoint intersects the control polygon in at most one location (excluding the endpoint). This condition guarantees that as the curve param eter increases, the angle between the position vector \(\overrightarrow{C(0) C(t)}\) and the initial tangent vector \(T=C^{\prime}(0)\) increases monotonically. Thus, the curve's sampled direction evolves in a single rotational direc tion and avoids reversal, as shown in Figure 4. Importantly, this condition can be efficiently verified via the \operatorname{conv}\left(\left.C_{1}\right|_{\left.s_{i}, s_{i+1}\right]}\right) \cap \operatorname{conv}\left(\left.C_{2}\right|_{\left[t_{j}, t_{j+1}\right]}\right)=\emptyset.
...
The separation vector \(\mathbf{D}\) is computed according to the local ge ometry at \(C(0)\), with three cases shown in Figure 5:

- a114 Case 1: Opposing curvature directions. If the two curves bend in opposite directions near \(C(0)\), we define:

$$
\mathbf{D}=\frac{\mathbf{T}_{1}}{\left\|\mathbf{T}_{1}\right\|}+\frac{\mathbf{T}_{2}}{\left\|\mathbf{T}_{2}\right\|},
$$
...
![Figure 6](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-6-p005.png)
...
![Figure 7](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-7-p005.png)
...
To ensure watertightness and topological consistency across ad jacent trimmed surfaces, we first perform precise boundary sam pling guided by bothparameter-domain topology and geometric intersections in \(\mathbb{R}^{3}\). Specifically:

![Figure 8](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-8-p006.png)
...
![Figure 9](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-9-p006.png)
...
$$
\operatorname{conv}\left(\left.S_{1}\right|_{R_{1, k}}\right) \cap \operatorname{conv}\left(\left.S_{2}\right|_{R_{2, p}}\right)=\emptyset .
$$
...
Our method can be generalized to non-manifold edges by ex tending the bidirectional mapping to a multi-way constraint. For surfaces \(S_{1}, S_{2}, S_{3}, \ldots\) meeting at an edge \(E\), we solve \(S_{1}\left(C_{1}\left(S_{i}\right)\right)=S_{2}\left(C_{2}\left(t_{j}\right)\right)=S_{3}\left(C_{3}\left(u_{k}\right)\right)=E\left(\xi_{l}\right)\) for a consistent \(\mathbb{R}^{3}\) discretization \(E\left(\xi_{1}\right)\). This ensures identical vertices across all patches for a wa tertight mesh. This method, however, requires valid B-rep topol-
...
Definition 2 (critical region). Let \(\mathrm{S}_{1}(u, v): \Omega_{1} \rightarrow \mathbb{R}^{3}\) and \(\mathrm{S}_{2}(s, t)\) : \(\Omega_{2} \rightarrow \mathbb{R}^{3}\) be two NURBS surface patches with \(S_{1}(u, v) \neq S_{2}(s, t)\) for all \((u, v) \in \Omega_{1},(s, t) \in \Omega_{2}\). A pair of domain partitions
...
$$
\mathrm{S}_{1}\left(\mathrm{C}_{1}\left(\mathrm{~S}_{\mathrm{i}}\right)\right)=\mathrm{S}_{2}\left(\mathrm{C}_{2}\left(\mathrm{t}_{i}\right)\right)=E\left(\xi_{\mathrm{i}}\right), \quad \forall i .
$$

Formally, if \(M_{1}\) is a triangulation of \(\Omega_{1}\) with \(\Delta \subset R_{1, k}\) for some \(k\) for every \(\Delta \in M_{1}\), and \(M_{2}\) is a triangulation of \(\Omega_{2}\) with \(\Delta \subset R_{2, p}\) for some \(p\) for every \(\Delta \in M_{2}\), then the spatial meshes

$$
\hat{M}_{1}=\left\{\Delta\left(S_{1}(P), S_{1}(Q), S_{1}(R)\right) \mid \Delta(P, Q, R) \in M_{1}\right\}
$$

![Figure 10](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-10-p007.png)
...
satisfy \(\hat{\Delta}_{1} \cap \hat{\Delta}_{2}=\emptyset\) for all \(\hat{\Delta}_{1} \in \hat{M}_{1}\) and \(\hat{\Delta}_{2} \in \hat{M}_{2}\).
...
![Figure 11](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-11-p008.png)
...
The density of the output mesh is governed by the quality met ric (e.g., minimal angle and maximal edge length), which directly determines the termination of the refinement process by control ling the insertion of new vertices. The algorithm exhibits a worst case time complexity of \(O\left(n^{2}\right)\), where \(n\) is the number of vertices in the final mesh, although its average-case performance is often closer to \(O(n \log n)\) in practice. Furthermore, the computational cost of key operations, such as point location and neighborhood queries during incremental insertion and edge flipping, can be sig nificantly reduced by employing spatial indexing structures like a quadtree, leading to substantial practical acceleration.
...
![Figure 12](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-12-p009.png)

*Figure 12: Case 3 for R 2 loops tessellation.*

![Figure 13](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-13-p009.png)

*Figure 13: Case 4 for R 2 loops tessellation.*

### 4.4 Mesh mapping and optimization

After obtaining the parameter domain mesh, we map it to \(\mathbb{R}^{3}\) space by evaluating the NURBS surface function at the ver tex coordinates of each triangle. This process transforms the 2D parameter-domain triangles into a set of 3D spatial tri angles that piecewise linearly approximate the original sur face geometry. Thanks to the constraints in previous steps such as the one-to-one correspondence of shared boundary points and the consistent cross-mapping of critical points-the resulting \(\mathbb{R}^{3}\) mesh is inherently watertight and topologically consistent.
...
![Figure 14](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-14-p010.png)
...
We evaluate our \(\mathbb{R}^{2}\) tessellation framework through a series of ex periments on individual curve segments and closed loops. This section presents both the constructed hierarchical bounding vol umes in the parametric domain and the resulting tessellated polylines guided by critical points. For comparison, we include three widely used strategies: uniform sampling (20 segments per curve's parameter domain), curvature-adaptive sampling, and arc-length-based sampling.
...
![Figure 15](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-15-p011.png)
...
![Figure 16](/Users/evanthayer/Projects/stepview/docs/2026_robust_tessellation_without_self_intersections/figures/figure-16-p012.png)
...
- From the definition of critical points, \(\mathrm-B}_-1, i} \cap \mathrm-~B}_-2, j}=\emptyset \) for all \(i \in \hat-\Delta}_-1} \cap \hat-\Delta}_-2}=\emptyset.

- Similarly, let \(\mathcal-T}^-\prime} \) be an arbitrary refinement of \(\mathcal-T} \): every original interval \(\left[\mathrm-t}_-j}, \mathrm-t}_-j+1}\right] \) (with \(\mathrm-t}_-j}, \mathrm-t}_-j+1} \in \mathcal-T} \)) is partitioned into \(\bigcup_-l=0}^-q_-j}-1}\left[\mathrm-t}_-j, l}, \mathrm-t}_-j, l+1}\right]

- Appendix B. Proof of Lemma 2

- Proof. Step 1: Vertices of 3D triangles lie in subpatch convex hulls take any \(\hat-\Delta}_-1} \in \hat-M}_-1} \) with 2 D preimage \(\Delta(\mathrm-P}, \mathrm-Q}, \mathrm-R}) \in \mathrm-M}_-1} \subseteq \mathrm-R}_-1, k^-*}} \). By definition of critical regions, \(P, Q, R \in R_-1, k^-*}} \) implies their images sat isfy \(S_-1}(P), S_-1}(Q), S_-1}(R) \in S_-1, k^-*}} \) (where \(S_-1, k^-*}}=\left.S_-1}\right|_-R_-1, k^-*}}} \)). By the convex hull property of NURBS surfaces (Piegl \& Tiller, 2012), \(\mathrm-S}_-1, k^-*}} \subseteq \mathrm-~B}_-1, k^-*}} \) (where \(\mathrm-B}_-1, k^-*}}=\operatorname-conv}\left(\mathrm-S}_-1, k^-*}}\right) \)), so all vertices of \(\hat-\Delta}_-1} \) lie in \(\mathrm-B}_-1, k^-*}} \). Step 2: Entire 3D triangles are contained in convex hulls. A 3D trian gle \(\hat-\Delta}_-1}=\Delta(A, B, C) \) (with \(A, B, C \in B_-1, k^-*}} \)) consists of all convex com binations of its vertices: L_-1}^-\prime}=\bigcup_-i=0}^-n-1} \bigcup_-k=0}^-p_-i}-1} \overline-C_-1}\left(s_-i, k}\right) C_-1}\left(s_-i, k+1}\right)}. \hat-\Delta}_-1}=\left\-\lambda_-1} A+\lambda_-2} B+\lambda_-3} C \mid \lambda_-1}, \lambda_-2}, \lambda_-3} \geq 0, \lambda_-1}+\lambda_-2}+\lambda_-3}=1\right\}. Since convex hulls are convex sets (closed under convex combi nations), \(\hat-\Delta}_-1} \subseteq B_-1, \mathrm-k}^-*}} \). Step 3: Non-intersection of 3D triangles. By definition of critical regions, \(B_-1, k} \cap B_-2, p}=\emptyset \) for all \(k \in\-1, \ldots, n\} \) and \(p \in\-1, \ldots, m\} \). For any \(\hat-\Delta}_-1} \subseteq B_-1, k^-*}} \) and \(\hat-\Delta}_-2} \subseteq B_-2, p^-*}} \), their containment in disjoint con vex hulls implies: By the same logic, any \(\hat-\Delta}_-2} \in \hat-M}_-2} \) is contained in some \(B_-2, p^-*}} \) (where \(B_-2, p^-*}}=\operatorname-conv}\left(S_-2, p^-*}}\right) \)). Conclusion All 3D triangles in \(\hat-M}_-1} \) are disjoint from all 3D triangles in \(\hat-M}_-2} \). Appendix C. Additional Evaluation on CAD Models This appendix presents evaluation results for 75 additional complex CAD B-rep models. Figures C1 and C2 display the visual tessellation results for all 75 models, along with statistical distributions of total computation time (Figure C3) and mesh quality metrics (Figures C4 and C5).
...
- 2525-20-20-15-Frequency Frequency 1510-10-5-0.50 |
