...
In this paper we study the ways in which the topology of the image of a polyhedron changes with changing viewpoint. We catalog the ways that the "topological appearance" or aspect can change. This enables us to find maximal regions of viewpoints of the same aspect. We use these techniques to construct the viewpoint space partition (VSP), a partition of viewpoint space into maximal regions of constant aspect, and its dual, the aspect graph. In this paper we present tight bounds on the maximum size of the VSP and the aspect graph and give algorithms for their construction, first in the convex case and then in the general case. In particular, we give tight bounds on the maximum size of \(\Theta\left(\mathbf{n}^{2}\right)\) and \(\Theta\left(\mathbf{n}^{6}\right)\) under ortho- graphic projection with a 2-D viewpoint space and of \(\Theta\left(\mathbf{n}^{3}\right)\) and \(\Theta\left(\mathbf{n}^{9}\right)\) under perspective projection with a 3-D viewpoint space. We also present properties of the VSP and aspect graph and give algo- rithms for their construction. * This work was supported in part by the NSF under grant DCR-8520870.
...
We call this partition of viewpoint space the viewpoint space partition or VSP. In the orthographic case it consists of a partition of the sphere, and in the perspective case it consists of a partition of \(\mathrm{R}^{3}\). The boundary points of the VSP are points from which an arbitrarily small change in viewpoint causes a change in the topology of the image. Figure 4 shows a tetrahedron and Figure 5 shows the structure of the viewpoint space partition generated by the tetrahedron under orthographic projection. The viewpoint space partition is flattened out onto a plane, and the shapes of the regions are not preserved, but the topological structure is preserved.
...
After all, the corresponding regions of viewpoint space seem to have the same relationship to each other in both cases: paths of viewpoints connecting a viewpoint in one region with a viewpoint in the other all pass through a single point \(P\) (see Figure 8).
...
The answer is a result of the fact that faces are not visible edge-on. The regions of viewpoint space in Figure 8 are shown with shading on open boundaries. Thus, the point P is in region A but not in regions \(\mathrm{AB}, \mathrm{AC}\), or ABC. Therefore it is possible to find a path of viewpoints from a point in region \(A\) to a point in region \(A B C\) which doesn't go through AB and AC, but it is not possible to find such a path between AB and AC.
...
objects are smooth, so they do not have any edges, unlike polyhedra. They study the topological structure of the singularities of the visual mapping, which occur at image points corresponding to world points that are tangent to or normal to the line of sight. If they had defined a model of visibility, they might have said that the only things that appear in an image are occluding contours and points at which the corresponding surface in the world is normal to the line of sight, i.e. local minima and maxima of distance.

In our model, we allow edges that are not occluding edges, but we require that faces be flat. We do not consider local minima and maxima of distance. The difference between the viewing models is illustrated in Figure 9, which shows an image of a cube under our viewing model and that of Koenderink and van Doorn.
...
Since the intersection of three general planes is a point, there can be at most \(O\left(n^{3}\right)\) intersection points of \(n\) planes. In order to get the largest possible VSP, we can assume that every set of three planes intersects in a distinct point. These intersection points are vertices of the VSP, so the VSP has \(O\left(n^{3}\right)\) vertices.

Thus each plane is cut into a subdivision of \(O\left(n^{2}\right)\) vertices, edges, and regions. Therefore among all \(\mathbf{n}\) planes there are \(O\left(\mathbf{n}^{3}\right)\) vertices, edges, and faces. A cell of the VSP is bounded by only four faces in the worst case, so the VSP has at most \(\mathrm{O}\left(\mathbf{n}^{3}\right)\) vertices, edges, faces, and cells. The aspect graph is the dual of the cell-face structure of the VSP, so its size is also \(\mathrm{O}\left(\mathbf{n}^{3}\right)\).
...
$$
\mathbf{p}=\mathbf{p}_{11}+t\left(\mathbf{p}_{12}-\mathbf{p}_{11}\right), 0 \leq t \leq 1
$$
...
$$
\mathbf{d}=\left[\left(\mathbf{p}-\mathbf{p}_{21}\right) \times\left(\mathbf{p}_{21}-\mathbf{p}_{22}\right)\right] \times\left[\left(\mathbf{p}-\mathbf{p}_{31}\right) \times\left(\mathbf{p}_{31}-\mathbf{p}_{32}\right)\right]
$$
...
$$
\begin{gathered} \theta=\tan ^{-1}\left(d_{x} / d_{z}\right) \\ \phi=-\sin ^{-1}\left(d_{y} / \sqrt{d_{x}^{2}+d_{y}^{2}+d_{z}^{2}}\right) \end{gathered}
$$

There are \(O\left(n^{3}\right)\) sets of three edges in a polyhedron, so there are \(O\left(n^{3}\right)\) such curves in viewpoint space.
...
The part of each grid that is visible behind the vertical screen is a portion of a nearlyvertical grid element and between 0 and \(\mathbf{m}\) nearly-horizontal grid elements. The view through a slit of the horizontal grid is symmetrical. Figure 21 contains close-up views of the part of the grids visible through the two slits.
...
Since every boundary of a region of constant aspect in viewpoint space is of the form of the edge-edge-edge boundaries defined above, a naive approach is to find every boundary in viewpoint space generated by every set of three object edges, taking those edges in all possible orders. Eq. (1) gives a potential boundary of visibility in viewpoint space for such a set of three edges. We can draw all \(O\left(n^{3}\right)\) of these boundaries on the sphere. It remains to find the subdivision of the sphere that they generate.
...
$$
\begin{aligned} & u=x_{0} \cos \theta-z_{0} \sin \theta \\ & v=x_{0} \sin \theta \sin \phi+y_{0} \cos \phi+z_{0} \cos \theta \sin \phi \end{aligned}
$$
...
$$
\begin{aligned} & \mathrm{x}(s)=\mathrm{x}_{1}+\mathrm{a}_{1} s \\ & \mathrm{y}(s)=\mathrm{y}_{1}+\mathrm{b}_{1} s \\ & \mathrm{z}(s)=\mathrm{z}_{1}+\mathrm{c}_{1} s \end{aligned}
$$
...
$$
\begin{gathered} \mathrm{u}=\left(\mathrm{x}_{1}+\mathrm{a}_{1} s\right) \cos \theta-\left(\mathrm{z}_{1}+\mathrm{c}_{1} s\right) \sin \theta \\ \mathrm{v}=\left(\mathrm{x}_{1}+\mathrm{a}_{1} s\right) \sin \theta \sin \phi+\left(\mathrm{y}_{1}+\mathrm{b}_{1} s\right) \cos \phi+\left(\mathrm{z}_{1}+\mathrm{c}_{1} s\right) \cos \theta \sin \phi \end{gathered}
$$
...
\(A_{3}\)-surface of aspect space corresponds to the visibility of a line in object space. The equations for such a surface for a line passing through the points \(\mathbf{p}_{1}=\) \(\left(x_{1}, y_{1}, z_{1}\right)\) and \(p_{1}+a_{1}=\left(x_{1}+a_{1}, y_{1}+b_{1}, z_{1}+c_{1}\right)\) were derived earlier and are repeated here:

$$
\begin{aligned} & \mathrm{u}=\left(\mathrm{x}_{1}+s \mathrm{a}_{1}\right) \cos \theta-\left(\mathrm{z}_{1}+s \mathrm{c}_{1}\right) \sin \theta \\ & \mathrm{v}=\left(\mathrm{x}_{1}+s \mathrm{a}_{1}\right) \sin \theta \sin \phi+ \\ & \quad\left(\mathrm{y}_{1}+s \mathrm{~b}_{1}\right) \cos \phi+\left(\mathrm{z}_{1}+s \mathrm{c}_{1}\right) \cos \theta \sin \phi \end{aligned}
$$

Thus, we can find the general form of \(a_{2}\)-surface by finding the intersection of two copies of Eqs. (11) and (12). However, we can find the equations more simply by noting that the viewing direction \(\mathbf{V}\) must be parallel to a vector from one of the lines to the other at the points where they appear to intersect, i.e. a vector from \(\mathbf{p}_{1}+s \mathbf{a}_{1}\) to \(\mathbf{p}_{2}+s_{2} \mathbf{a}_{2}\) (see Figure 25).
...
Using \(\mathbf{V}=\left(v_{\mathbf{x}}, v_{\mathbf{y}}, v_{\mathbf{z}}\right)\) for the viewpoint, we get

$$
\frac{\mathrm{v}_{\mathrm{x}}}{\mathrm{v}_{\mathrm{z}}}=\frac{\left(\mathrm{x}_{2}+s_{2} \mathrm{a}_{2}\right)-\left(\mathrm{x}_{1}+s \mathrm{a}_{1}\right)}{\left(\mathrm{z}_{2}+s_{2} \mathrm{c}_{2}\right)-\left(\mathrm{z}_{1}+s \mathrm{c}_{1}\right)}
$$

From these equations we can find \(u\) and \(v\) as a function of \(\theta\) and \(\phi\). The surface can be represented by twelve constants: \(a_{1}, a_{2}, b_{1}, b_{2}, c_{1}, c_{2}, x_{1}, x_{2}, y_{1}, y_{2}, z_{1}, z_{2}\). These are related to the coordinates of the four endpoints of the two lines involved in the visual event.

The intersection of three 3-surfaces of aspect space is \(a_{1}\)-surface or curve. It corresponds to the visual event of the apparent intersection of three object edges in an image. Three object edges can appear to intersect from \(a_{1}\)-D curve of viewpoints, and their apparent intersection is a single point, so the aspect representation for this visual event is \(a_{1}\)-surface or curve in aspect space.

However, there is a simpler way to find the form. Consider three edges in space, \(\mathbf{p}_{1}\) to \(\mathbf{p}_{1}+\mathbf{a}_{1}, \mathbf{p}_{2}\) to \(\mathbf{p}_{2}+\) \(\mathbf{a}_{2}\), and \(\mathbf{p}_{3}\) to \(\mathbf{p}_{3}+\mathbf{a}_{3}\). Pick a point \(\mathbf{p}_{1}+s \mathbf{a}_{1}\) on one line (see Figure 26) and find a line through that point that intersects both of the other lines. Pital
...
$$
\frac{\mathrm{v}_{\mathrm{y}}}{\mathrm{v}_{\mathrm{z}}}=\frac{\left(\mathrm{y}_{2}+s_{2} \mathrm{~b}_{2}\right)-\left(\mathrm{y}_{1}+s \mathrm{~b}_{1}\right)}{\left(\mathrm{z}_{2}+s_{2} \mathrm{c}_{2}\right)-\left(\mathrm{z}_{1}+s \mathrm{c}_{1}\right)}
$$

Solving for s yields

$$
s=\frac{\mathbf{V} \cdot\left[\left(\mathbf{p}_{2}-\mathbf{p}_{1}\right) \times \mathbf{a}_{2}\right]}{\mathbf{V} \cdot\left(\mathbf{a}_{1} \times \mathbf{a}_{2}\right)}
$$

Note the viewing direction must be along the intersection of the planes defined by the point \(\mathbf{p}_{1}+s \mathbf{a}_{1}\) and each of the other two lines. Normals to these planes are given by
...
$$
\mathbf{V}^{\prime}=\left(\left(\mathbf{p}_{1}+s \mathbf{a}_{1}-\mathbf{p}_{2}\right) \times \mathbf{a}_{2}\right) \times\left(\left(\mathbf{p}_{1}+s \mathbf{a}_{1}-\mathbf{p}_{3}\right) \times \mathbf{a}_{3}\right)
$$
...
In order to find the viewing direction at which this visual event occurs, consider four lines in space, through \(\mathbf{p}_{1} \& \mathbf{p}_{1}+\mathbf{a}_{1}, \mathbf{p}_{2} \& \mathbf{p}_{2}+\mathbf{a}_{2}, \mathbf{p}_{3} \& \mathbf{p}_{3}+\mathbf{a}_{3}\), and \(\mathbf{p}_{4} \& \mathbf{p}_{4}+\mathbf{a}_{4}\). Pick a point \(\mathbf{p}_{1}+s \mathbf{a}_{1}\) on one line (see Figure 27) and suppose a line through that point intersects all three of the other lines.
...
It is perhaps surprising that the asp, which represents the appearance of an object from every viewpoint, has smaller size than the VSP, which only represents the regions of constant aspect in viewpoint space. The asp has size \(O\left(\mathbf{n}^{4}\right)\) while the VSP has size \(\mathrm{O}\left(\mathbf{n}^{6}\right)\). The asp is smaller for a reason analogous to the reason that a polyhedron of size \(\mathbf{n}\) can have an image of size \(\mathrm{O}\left(\mathbf{n}^{2}\right)\). Forming the image of the polyhedron requires projecting lines in \(R^{3}\) into \(R^{2}\), and lines that did not intersect in \(R^{3}\) may intersect in \(R^{2}\). A set of \(\mathbf{n}\) lines can have \(\mathrm{O}\left(\mathbf{n}^{2}\right)\) intersection points. Similarly, projecting the \(\mathrm{O}\left(\mathbf{n}^{3}\right)\) 1-surfaces of the asp into viewpoint space causes them to intersect where they did not intersect before, resulting in \(O\left(n^{6}\right)\) intersection points.
...
Therefore the VSP is a partition of \(R^{3}\). It has cells corresponding to volumes of viewpoint space of constant aspect. Since we are modelling "looking at" an object that is centered on the origin, the viewing direction is again the direction from the viewpoint to the origin.
...
Figure 29 presents a polyhedral scene with three screens on three adjacent sides of a cube. There are three grids, each one a distance behind one of the screens. We will show that there are \(\Omega\left(\mathbf{n}^{9}\right)\) different aspects from viewpoints inside the cube.
...
$$
\begin{aligned} & u=\frac{f\left(x_{0} \cos \theta-z_{0} \sin \theta\right)}{r-\left(x_{0} \sin \theta \cos \phi-y_{0} \sin \phi+z_{0} \cos \theta \cos \phi\right)} \\ & v=\frac{f\left(x_{0} \sin \theta \sin \phi+y_{0} \cos \phi+z_{0} \cos \theta \sin \phi\right)}{r-\left(x_{0} \sin \theta \cos \phi-y_{0} \sin \phi+z_{0} \cos \theta \cos \phi\right)} \end{aligned}
$$
...
$$
\mathbf{a}_{1}+s_{1} \mathbf{a}_{2}+r_{1}\left[s_{1}^{2} \mathbf{a}_{3}+s_{1} \mathbf{a}_{4}+\mathbf{a}_{5}\right]=\mathbf{a}_{6}+s_{2} \mathbf{a}_{7}+r_{2}\left[s_{2}^{2} \mathbf{a}_{8}+s_{2} \mathbf{a}_{9}+\mathbf{a}_{10}\right]
$$
...
The aspect graph, the dual of the cell-face structure, is constructed in linear time in the size of the VSP in the same manner as in the convex case. The aspect from some viewpoint requires \(O\left(n^{2}\right)\) time to compute and \(O\left(n^{2}\right)\) space to represent, so storing the aspect at each vertex of the aspect graph in the perspective case
