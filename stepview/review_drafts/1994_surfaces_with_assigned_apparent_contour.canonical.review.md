...
In all the paper we will denote by \(\mathbf{S}^{1}\) the unit circle in Euclidean plane \(\mathbb{R}^{2}\), and by \(\coprod_{h=1}^{k} \mathbf{S}^{1}\) the disjoint union of \(k\) identical copies of the circle; all surfaces and maps will be supposed smooth (i.e. of class \(C^{\infty}\) ). Furthermore we will often use "cut and paste" techniques which work fine in \(C^{0}\) or PL category. Since in low dimension these categories are the same as the \(C^{\infty}\) one, we shall not care to give technical details.
...
$$
R_{X}\left(\left.F\right|_{\partial S}\right)=\chi(S)-\beta \chi(\mathbf{N})
$$
...
- for any two points we have j j' if and only if with respect to the ordering induced by the orientation on ri. ~
...
We call weighted genus of a \(\beta\)-assemblage the weighted genus of its associated graph, and we denote it by \(h(\mathbf{x})\).
...
\(\mathfrak{A}(F)\), and the mapping \(F \mapsto \mathbf{A}(F)\) is a bijection \(E_{\infty}^{0}(f) / \underset{\sim}{r} \rightarrow \mathcal{A}_{m}^{0}(w)\).

Let \(F \in E_{\infty}^{\beta}(f)\) and let \(D_{1}, \ldots, D_{\beta} \subset \mathbf{N}\) be the disks bounded by the curves \(g_{l}\) defining the \(\beta\)-expansion of \(f\) (see definition 0.10), call \(\left\{p_{1}, \ldots, p_{\beta}\right\}=F^{-1}(\infty)\) and \(U_{i}^{j}\) the interior of the connected component of \(F^{-1}\left(D_{i}\right)\) containing \(p_{j}\). For each \(\sigma \in \sigma_{\beta}\), let \(S_{\sigma}=S-\bigcup_{i} U_{i}^{\sigma i}\). Clearly \(F_{\sigma}=\left.F\right|_{S_{\sigma}} \in E_{\infty}^{0}\left(f^{\beta}\right)\), hence, by the case \(\beta=0\), we get an assemblage \(\mathrm{A}\left(F_{\sigma}\right) \in \mathcal{A}_{m}^{0}\left(w^{\beta}\right)=\mathcal{A}_{m}^{\beta}(w)\). Such a construction actually defines a bijection between \(E_{\infty}^{\beta}(f) /{ }_{\sim}\) and \(\mathcal{A}_{m}^{\beta}(w) / \zeta_{\beta}\).
...
REMARK 1.5. It is obvious that \(f_{D}^{*}=f_{\mathcal{C}-D}^{*}\) and \(f_{D}^{* *}=f_{\mathcal{C}-D}^{* *}\). By local arguments, it is easily seen that if \(F: \mathbf{S}^{1} \times[-1,1] \rightarrow \mathbf{N}\) is an excellent mapping such that \(\Sigma_{F}=\mathbf{S}^{1}=\mathbf{S}^{1} \times\{0\}\) and \(\left.F\right|_{\Sigma_{F}}=f\), then there exists a Let \(f: \coprod_{h=1}^{k} \mathbf{S}^{1} \rightarrow \mathbf{N}\) be a sided \(\mathrm{CN} k\)-curve, let \(D\) be as above and let \(H \subseteq\{1, \ldots, k\}\); we call such a pair ( \(D, H\) ) a deformation pair. Denote by \(\bar{H}\) the complement of \(H\). \(f_{(D, H)}=\left\{\left(f_{h}\right)_{D}^{*} \mid h \in H\right\} \amalg\left\{\left(f_{h}\right)_{D}^{* *} \mid h \in \bar{H},\right\}\), where \(k_{H}=2 \# H+\# \bar{H}\).
...
$$
2 R_{X}\left(\left.F\right|_{\Sigma}\right)-c=\chi(S)-\beta \chi(\mathbf{N}) ;
$$
...
REMARK 1.27. Let \(\tilde{w}_{(D, H)}\) the word for \(f_{(D, H)}\) defined by \(R\); clearly \(\tilde{w}_{(D, H)}\) is obtained by renumbering the second index of all letters in \(w_{(D, H)}\) in such a way to preserve the previous ordering of letters belonging to the same segment; hence, a \(\beta\)-assemblage \(\mathbf{A}=((D, H), \mathrm{A})\) for the word \(w\) obviously defines a Proposition 1.28. The mapping \(\mathbf{A} \mapsto \tilde{\mathbf{I}}_{\mathbf{A}}\) is a bijection between the set of \(\beta\)-assemblages of \(w\) having ( \(D, H\) ) as deformation pair and \(\mathcal{A}^{\beta}\left(\tilde{w}_{(D, H)}\right)\). Furthermore \(\mathbf{A}\) and \(\tilde{\mathbf{a}}_{\mathbf{A}}\) have the same number of components and weighted genus.

Proposition 1.29. Let \(f\) be a \(C N_{k}\)-curve and \(w\) be a word defined by a system of segments. Then, for all \(\mathbf{A} \in \mathcal{A}^{\beta}(w)\),

$$
2 R_{X}(f)-c_{-}+c_{+} \geq 2 l(\mathbf{A})-k_{H}-h(\mathbf{A})-\beta \chi(\mathbf{N}) .
$$

PROOF. Use Proposition 1.28, (0.2) and Lemma 1.14. D definition 1.30. Let \(w\) be a word for the CN \(k\)-curve \(f\), defined by a system of segments. A \(\beta\)-assemblage \(\mathbf{A}\) of \(w\) will be called minimal if in (1.2) equality holds. We denote by \(A_{m}^{\beta}(w)\) the set of minimal \(\beta\)-assemblages of \(w\).

REMARK 1.31. The mapping \(\mathbf{A} \mapsto \tilde{\mathbf{2}}_{\mathbf{A}}\) is a bijection between the set of minimal \(\beta\)-assemblages of \(w\), having ( \(D, H\) ) as deformation pair, and \(\mathcal{A}_{m}^{\beta}\left(\tilde{w}_{(D, H)}\right)\).

Let \(f\) be a CN \(k\)-curve, and suppose that the \(h^{\text {th }}\) component of \(f\) has no cusps. Let \((D, H)\) be a deformation pair; then \(\left(w_{h}\right)_{D, 1}^{*}=\left(w_{h}\right)_{D, 2}^{*}=w_{h}\). We definition 1.32. We say \(\mathbf{A}\) is \(\iota\)-equivalent to \(\mathbf{A}^{\prime}\) (briefly \(\mathbf{A} \stackrel{\iota}{\sim} \mathbf{A}^{\prime}\) ) if there exist involutions \(\iota_{h_{1}}, \ldots, \iota_{h_{n}}\) such that \(\mathbf{A}^{\prime}=\iota_{h_{n}} \ldots \iota_{h_{1}} \mathbf{A}\). We say that As in §0, we have an action of the symmetric group \({ }_{\beta}\), on the set \(A^{\beta}(w)\), given by \(\sigma \mathbf{A}=((D, H), \sigma \mathbb{\mathrm { c }})\). We denote by \(\sim\) the equivalence relation on \(\mathcal{A}^{\beta}(w)\) REMARK 1.33. If \(\mathbf{A} \sim \mathbf{A}^{\prime}\) then \(\mathbf{A}\) and \(\mathbf{A}^{\prime}\) have the same number of components and the same weighted genus. Hence \(\sim\) can be restricted to the set
...
Lemma 1.35. Let \(\mathbf{A}, \mathbf{A}^{\prime} \in \mathcal{A}^{\beta}(w), \sigma \in \sigma_{\beta}\). If \(\mathbf{A} \stackrel{\iota}{\sim} \mathbf{A}^{\prime}\), then \(\sigma \mathbf{A} \stackrel{\iota}{\sim} \sigma \mathbf{A}^{\prime}\). If REMARK 1.36. Denote by \(\stackrel{\iota \delta}{\sim}\) the equivalence relation generated by \(\stackrel{\iota}{\sim}\) and \(\stackrel{\delta}{\sim}\). We can consider the quotient relation over \(\mathcal{A}^{\beta}(w) / \boldsymbol{\zeta}_{\beta}\); we denote it by \(\stackrel{\iota \delta}{\sim}\) again. Denote by \([\mathbf{A}]\) the class of \(\mathbf{A}\) in \(\mathcal{A}^{\beta}(w) /_{6_{\beta}}\); by Lemma 1.35 we have that [AI'6 [A'] \(c_{3}\) u A'; furthermore.~m(w),~ _ (.~~(w) ~a) a.

Remark 1.37. Let \((D, H)\) be a deformation pair and \(\tilde{w}_{(D, H)}\) be the word for \(f_{(D, H)}\) defined by the same segments as \(w\). Let \(\mathbf{A}=((D, H), \mathbf{A}) \in \mathcal{A}^{\beta}(w)\) and Proof of Theorem 1.34. We start trying to define a map \(\mathbf{A}_{1}^{\beta}: E_{\infty}^{\beta}(f) \rightarrow\) \(\mathcal{A}_{m}^{\beta}(w) / \zeta_{\beta}\). Let \(F \in E_{\infty}^{\beta}(f)\), choose a tubular neighborhood \(U\) of the set of critical points of \(F\), in such a way that \(\left.F\right|_{\partial(S-U)}=f_{(D, H)}\) for some choice of \((D, H)\). Clearly \(\left.F\right|_{S-U}\) is an immersion which estends \(f_{(D, H)}\) and attains exactly \(\beta\) times the value \(\infty\). Let \(\mathfrak{A}^{\beta}(F) \in \mathcal{A}_{m}^{\beta}\left(\tilde{w}_{(D, H)}\right) / \xi_{\beta}\) be the class of minimal assemblages associated to \(\left.F\right|_{S-U}\) by Theorem 0.18. define \(\mathbf{A}_{1}^{\beta}(F) \in \mathcal{A}_{m}^{\beta}(w) / \zeta_{\beta}\) as the unique class such that \(\left[\tilde{\mathbf{A}_{1}}\right]_{\mathbf{A}_{1}^{\beta}(F)}=\mathfrak{A}^{\beta}(F)\) (Remark 1.37). Observe that such a construction is not univocal, in fact it depends on:
...
Let \(U\) be the tubular neighborhood of \(\Sigma\), and let \(\gamma\) be an arc of \(\left(\left.F\right|_{\left(s-\bigcup \dot{D}_{i}\right)}\right)^{-1}(R)\). Clearly, \(\gamma\) intersects \(\partial U\) in as many points as \(\gamma \cap \Sigma\). Label these points as the corresponding ending point of \(\gamma\). If we read the letters on \(\partial U\) we get the word \(w_{(D, H)}\), while if we read the letters on \(\partial D_{i}\) we get the words \(u_{i}\) (for all \(i=1, \ldots, k\) ) of the \(\beta\)-expansion (see \(\S 0\) ). Pairing together letters corresponding to ending points of the same arc, we get a \(\beta\)-assemblage for \(w_{(D, H)}\), hence a \(\beta\)-assemblage for \(w\), whose class in \(\mathcal{A}^{\beta}(w) /{ }_{\zeta_{\beta}}\) is equal to \(\mathbf{A}_{1}(F)\) (compare with the sketch of proof of Theorem 0.18).
...
Proof. By contradiction, suppose that \(f\) has a positive cusp; then every deformation of \(f\) has a positive curl, and then, by Lemma 0.22, \(\mathcal{A}_{m}^{\beta}\left(\widetilde{w}_{(D, H)}\right)=\varnothing\) for all deformation pairs ( \(D, H\) ); by Remark 1.31, \(\mathcal{A}_{m}^{\beta}(w)=\varnothing\).
...
\(M\) and \(\widetilde{S}\) can be pasted along the boundary, so that the map \(F\) obtained pasting \(F_{1}\) and \(F_{2}\) is an excellent map extending \(f\) and, by construction, \(\mathbf{A}^{\beta}(F)=\mathbf{A}\).
...
Proposition 2.8. Let \(C_{1}, C_{2} \in \mathcal{C}\), then \(F\left(C_{1}\right) \cap F\left(C_{2}\right)\) is connected.
...
Factorization. Let \(S\) and \(\mathbf{N}\) be surfaces and \(\mathbf{E} \xrightarrow{\pi} \mathbf{N}\) a line bundle.
...
We now fix our attention on finding factorizability conditions for excellent mappings. Let \(F^{-1}(\infty)=\left\{p_{1}, \ldots, p_{\beta}\right\}\) and let \(D_{1}, \ldots, D_{\beta}\) be disks in \(S\) such that \(p_{i} \in D_{i}\) and \(\left.F\right|_{\partial D_{i}}=g_{i}\) ( \(g_{i}\) as in definition 0.10), and let \(\tilde{S}=S-\bigcup_{i=1}^{\beta} \dot{D}_{i}\),

Proposition 2.12. \(F\) is factorizable into \(\mathbf{E}\) if and only if \(\widetilde{F}\) is.

Proof. Let \(\tilde{F}_{1}: \tilde{S} \rightarrow \mathbf{E}\) be an embedding such that \(\pi \circ \tilde{F}_{1}=\tilde{F}\), and \(U_{\infty}\) an open ball around \(\infty\) such that \(U_{\infty} \cap \Gamma=\varnothing\), and \(U_{\infty}\) contains all curves \(g_{i}\). Let \(U_{i}\) we may suppose that \(h\) assumes constant value \(c_{i}\) on \(U_{i}-D_{i}\), such that \(c_{i} \neq c_{j}\) for all \(i \neq j\). Extend \(\widetilde{F}_{1}\) to the required embedding \(F_{1}\), defining \(F_{1}(p)=\Phi^{-1}\left(F(p), c_{i}\right)\) The trivial bundle. Let us first consider the case \(\mathbf{E}=\mathbf{N} \times \mathbb{R}\). In Remark 2.10 Proposition 2.13. Let \(h\) be an height function for \(F\); then for all \(C_{1}\), \(C_{2} \in C\) such that \(F\left(C_{1}\right) \cap F\left(C_{2}\right) \neq \varnothing\), one (and only one) of the following two holds:

$$
\begin{array}{ll} \forall x \in C_{1}, y \in C_{2} & F(x)=F(y) \Rightarrow h(x)<h(y) ; \\ \forall x \in C_{1}, y \in C_{2} & F(x)=F(y) \Rightarrow h(x)>h(y) . \end{array}
$$
...
Proof. Let \(\widetilde{S}\) be as in Proposition 2.12. We proceed in two steps: Step 1: definition of a height on \(\widetilde{S}\) minus a tubular neighborhood of \(\Sigma\); Step 2: extension of such a height to the tubular neighborhood. The thesis will follow by Proposition 2.12.
...
$$
h_{1}: x \longmapsto \begin{cases}s & \text { if } x \in C_{s}^{*} \\ s(\ell)+(t(\ell)-s(\ell)) \psi\left(\varphi_{r(\ell)}(F(x))\right) & \text { if } x \in U_{\ell}\end{cases}
$$
...
$$
M= \begin{cases}\mathbb{R} \times[-1,1] /(x, y)=(x+1, y) & \text { if } U \text { is orientable, } \\ \mathbb{R} \times[-1,1] /(x, y)=(x+1,-y) & \text { if } U \text { is non-orientable. }\end{cases}
$$
...
$$
m(\bar{t}) \leq h_{1}(t, y) \leq M(\bar{t}) \quad \forall(t, y) \in[\bar{t}-\delta, \bar{t}+\delta] \times[-1,1] ;
$$

and, up to taking \(U\) and \(\delta\) small enough, that:

$$
F\left(\left[\bar{t}_{1}-\delta, \bar{t}_{1}+\delta\right] \times[-1,1]\right) \cap F\left(\left[\bar{t}_{2}-\delta, \bar{t}_{2}+\delta\right] \times[-1,1]\right)=\varnothing .
$$
...
we see that finding a section of \(\widetilde{F}^{*} \mathbf{E}_{\rho}\) is the same as finding a function \(h: S^{\prime} \rightarrow \mathbb{R}\), such that:

$$
h\left(x_{1}\right)=\rho_{i} h\left(x_{2}\right) \text { if } F\left(\xi\left(x_{1}\right)\right)=F\left(\xi\left(x_{2}\right)\right) \in \omega_{i} \quad \forall x_{1}, x_{2} \text { s.t. } \xi\left(x_{1}\right)=\xi\left(x_{2}\right)
$$

and satisfying the anologous conditions on the differentials. We will denote by \(\sigma_{h}\) the section of \(\widetilde{F}^{*} \mathbf{E}_{\rho}\) defined by \(h\). Clearly the following holds:
...
We will call such a function \(h\) a strange height for the mapping \(F_{\tilde{c}}\). Let us introduce some more notation. Let \(\tilde{H}(F)=(H(F) \cap \tilde{S}) \cup \partial \tilde{S}\), and \(H^{\prime}(F) \subset S^{\prime}\) be the set \(H^{\prime}(F)=\xi^{-1}(\tilde{H}(F))\). Denote by \(\mathcal{C}\) the set of connected components of \(S-H(F)\), by \(\tilde{C}\) the set of components of \(\tilde{S}-\tilde{H}(F)\) and finally by \(C^{\prime}\) the set of components of \(S^{\prime}-H^{\prime}(F)\). It is immediate that inclusion \(\widetilde{S} \hookrightarrow S\) gives a bijection between \(\tilde{C}\) and \(\mathcal{C}\); while \(\underset{\sim}{\xi}: S^{\prime} \rightarrow \tilde{S}\) gives a bijection between \(\mathcal{C}^{\prime}\) and \(\tilde{\mathcal{C}}\). For all \(C \in \mathcal{C}\) denote by \(\tilde{C} \in \tilde{\widetilde{C}}\) and by \(C^{\prime} \in \mathcal{C}^{\prime}\) the corresponding components (i.e. \(\widetilde{C}\) is the interior of \(C \cap \widetilde{S}\) and \(C^{\prime}\) is the component such that Proposition 2.26. Let \(C_{1}, C_{2} \in \mathcal{C}\); then \(F\left(C_{1}\right) \cap F\left(C_{2}\right) \neq \emptyset\) if and only if Proof. The second equivalence is immediate, the first follows from the fact that if \(F\left(C_{1}\right) \cap F\left(C_{2}\right) \neq \varnothing\) and \(F\left(\tilde{C}_{1}\right) \cap F\left(\tilde{C}_{2}\right)=\varnothing\) then \(F\left(C_{1}\right) \cap F\left(C_{2}\right) \subset U_{\infty}\) and the fact that \(\partial\left(F\left(C_{1}\right) \cap F\left(C_{2}\right)\right)\) contains branches of \(\Gamma\), while \(U_{\infty} \cap \Gamma=\varnothing\).
...
Proposition 2.28. Let \(C_{1}^{\prime}, C_{2}^{\prime} \in \mathcal{C}^{\prime}\) be such that \(F^{\prime}\left(C_{1}^{\prime}\right) \cap F^{\prime}\left(C_{2}^{\prime}\right) \neq \emptyset\). One of the following two holds:

$$
\begin{aligned} & \forall x \in C_{1}^{\prime}, y \in C_{2}^{\prime} \quad \text { if } \quad F(x)=F(y) \quad \text { then } \quad h(x)<h(y) ; \\ & \forall x \in C_{1}^{\prime}, y \in C_{2}^{\prime} \quad \text { if } \quad F(x)=F(y) \quad \text { then } \quad h(x)>h(y) . \end{aligned}
$$
...
$$
\begin{aligned} & \gamma_{i}^{\prime}(1)=x_{i}^{\prime} \text { and } \gamma_{i}^{\prime \prime}(1)=x_{i}^{\prime \prime} \quad i=1,2 ; \\ & \forall t \neq 1 \quad \begin{cases}\gamma_{1}^{\prime}(t) \in C_{1}^{\prime}, & \gamma_{1}^{\prime \prime}(t) \in C_{2}^{\prime} \\ \gamma_{2}^{\prime}(t) \in C_{3}^{\prime}, & \gamma_{2}^{\prime \prime}(t) \in C_{4}^{\prime}\end{cases} \\ & \forall t \quad\left\{\begin{array}{l} F^{\prime}\left(\gamma_{1}^{\prime}(t)\right)=F^{\prime}\left(\gamma_{2}^{\prime}(t)\right), \\ F^{\prime}\left(\gamma_{1}^{\prime \prime}(t)\right)=F^{\prime}\left(\gamma_{2}^{\prime \prime}(t)\right) . \end{array}\right. \end{aligned}
$$
