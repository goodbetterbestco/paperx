...
![Figure 1. Cad model of a screw with flaws. Free edges are highlighted in blue . (For interpretation of the references to color in this figure](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-1-p002.png)
...
$$
\begin{equation*} \Omega=\left\{B_{i} \mid i \in\{1, \ldots, n\}\right\} \tag{1} \end{equation*}
$$

![Figure 2. Example topology with n = 5 vertices, m = 7 edges, and o = 3 faces.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-2-p005.png)
...
with \(n\) being the number of volumetric bodies. For simplicity of presentation, we assume that a B-Rep model consists only of one domain \(\Omega=B\). A B-Rep body consists of topology \(T\) and geometry \(G\) [6]:

$$
\begin{equation*} B(T, G) \tag{2} \end{equation*}
$$
...
\begin{align*} & \boldsymbol{P}_{i}=\left(x_{i}\binom{y_{i}, z_{i}}{x(\xi)}^{\mathrm{T}}\right. \tag{9}\\ & \boldsymbol{C}_{i}(\xi)=\underset{z(\xi)}{y(\xi)} \end{align*} \quad \text { e.g., } \quad \boldsymbol{C}_{i}(\xi)=N_{j}^{n \boldsymbol{\Sigma}} N_{j}(\xi) \cdot \boldsymbol{Q}_{j}
...
![Figure 3. At each edge e; two boundary curves Cs, and C5, meet.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-3-p006.png)
...
\boldsymbol{S}_{i}(\boldsymbol{\xi})=\left(\begin{array}{c} x(\boldsymbol{\xi}) \tag{11}\\ y(\boldsymbol{\xi}) \\ z(\boldsymbol{\xi}) \end{array} \quad e. g., \quad \boldsymbol{S}_{i}(\boldsymbol{\xi})={ }_{j}^{n \boldsymbol{\Sigma}} \geq \sum_{k} N_{j}(\xi) \cdot N_{k}(\eta) \cdot \boldsymbol{Q}_{j, k}\right.
...
$$
\begin{align*} & P=\left\{\boldsymbol{P}_{i} \mid i=\{1, \ldots, n\}\right\} \tag{12}\\ & C=\left\{\boldsymbol{C}_{i} \mid i=\{1, \ldots, 2 \cdot m\}\right\} \tag{13}\\ & S=\left\{\boldsymbol{S}_{i} \mid i=\{1, \ldots, o\}\right\} \tag{14} \end{align*}
$$
...
$$
\begin{equation*} F^{S T L}=\left\{f_{i}\left|i \in\{1, \ldots, o\}, f_{i}=\left(v_{\alpha}, v_{\beta}, v_{\gamma}\right), \boldsymbol{t}_{i}, v_{\kappa} \in V,|V|=3 \cdot o\right\}\right. \tag{15} \end{equation*}
$$
...
![Figure 4. Vertex with adjoined edges and faces](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-4-p007.png)
...
![Figure 5. Gaps between trimmed NURBS patches of the Utah teapot.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-5-p007.png)
...
![Figure 6. Topological flaws.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-6-p008.png)

*Figure 6. Topological flaws.: Topological flaws.*

![Figure 7. Geometrical flaws.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-7-p008.png)
...
$$
\begin{equation*} \omega^{t_{i}}\left(T^{t_{i}}, G^{t_{i}}\right) \subset B \tag{16} \end{equation*}
$$

![Figure 8. Hybrid flaws which consist of topological and geometrical components.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-8-p009.png)
...
![Figure 9. UML-diagram of a possible object-oriented B-Rep implementation.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-9-p009.png)
...
Face 0..* id: int boundaryEdges : list<Edge> normal: vector<double> adjacentFaces : list<Face> geometry: Surface Surface 1 0.* Edge 0.* id: int vertice : list< Vertex> geometry: curve curve 2 2 Vertex id: int geometry: point point 1

![Figure 10. B-Rep sub-part wi, which corresponds to face fi and consists of the topology Tfi and the corresponding geometry Gfi.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-10-p010.png)
...
$$
\begin{equation*} O^{\text {select }}\left(B, t_{i}\right) \mapsto \omega^{t_{i}} \tag{17} \end{equation*}
$$
...
- Let OshallowCopy be a shallow copy operator that copies an arbitrary entity a;. For a more detailed description of the object-oriented concept of 'shallow' and 'deep' copying see, e.g., [51]. a; can be a topological entity ti, a geometrical entity gi, or an internal ri" or external relation rex

$$
\begin{equation*} O^{\text {shallowCopy }}\left(a_{i}\right) \mapsto \tilde{a}_{i}, \text { where } \tilde{a}_{i}:=a_{i} \tag{19} \end{equation*}
$$
...
$$
\begin{gather*} \text { ll as the internal adjacency relations. } \\ O^{\text {deepCopy }}\left(\underset{\sigma^{t_{i}}}{\omega^{t_{i}}} \mapsto \tilde{\omega}^{t_{i}}\left(\tilde{T}^{t_{i}}, \tilde{G}^{t_{i}}\right), \text { with } \tilde{T}^{t_{i}} \tilde{\tau}, \tilde{\rho}^{\text {int }}\right), \tilde{\tau}_{i}:=O^{\text {shallowCopy }}\left(\tau_{i}\right), \\ \tilde{\rho}_{i}^{\text {int }}:=O^{\text {shallowCopy }}\left(\rho_{i}^{\text {int }}\right) \forall \tau_{i}, \rho_{i}^{\text {int }} \in T^{t_{i}}, \tag{20} \end{gather*}
$$
...
$$
\begin{equation*} O^{\text {delete }}\left(B, f_{i}, g^{f_{i}}\right) \mapsto \tilde{B}(\tilde{T}, \tilde{G}), \text { where } \tilde{F}=F \backslash f_{i} ; \tilde{G}=G \backslash g^{f_{i}}, \delta<\varepsilon \tag{21} \end{equation*}
$$
...
$$
\begin{equation*} O^{\text {explode }}(B)=O^{\text {join }} B^{\circ}, O^{\text {deepCopy }} O^{\text {extract }}(B, F) \mapsto \tilde{B}, \text { where } \tilde{r}^{\text {ext }}=\emptyset \tag{22} \end{equation*}
$$

where \(B^{\circ}\) is an empty body.

O^{f l i p}\left(\begin{array}{l} f_{i} \tag{23}\\ f_{i} \end{array} \mapsto \tilde{f}_{i}, \text { where } \tilde{f}_{i}=\left(\left(e_{\kappa}\right), \tilde{\boldsymbol{n}}_{i}=-1 \cdot \boldsymbol{n}_{i}\right)\right.

- Let Omove be a geometric move operation that moves the point Pi within the range e. Additionally, all adjoined surfaces and curves are adapted consistently such that they form \(a_{2}\)-manifold without boundaries ce a operation. This involves the following adaptions to the adjoined surfaces sP; = 15! 1 and curves
...
\begin{align*} & \quad \exists \boldsymbol{\xi}: \tilde{\boldsymbol{S}}_{i}^{\tilde{\boldsymbol{P}}_{i}}(\boldsymbol{\xi})=\tilde{\boldsymbol{P}}_{i} \forall \tilde{\boldsymbol{S}}_{i}^{\tilde{\boldsymbol{P}}_{i}}, \exists \zeta: \tilde{\boldsymbol{C}}_{i}(\zeta)=\tilde{\boldsymbol{P}}_{i} \forall \tilde{\boldsymbol{C}}_{i}^{\tilde{\boldsymbol{P}}_{i}}, \tag{24}\\ & \text { and }( \\ & \quad \text { dist } \tilde{\boldsymbol{C}}_{A}^{e_{k}}, \tilde{\boldsymbol{C}}_{B}^{e_{k}}=0 \forall\left(\tilde{\boldsymbol{S}}_{A}^{e_{k}}, \tilde{\boldsymbol{S}}_{B}^{e_{k}}\right) \text { at } v_{i} \end{align*}
...
$$
\begin{gather*} O^{\text {detach }}\left(G, e_{k}\right) \mapsto \tilde{G}, \text { whene dist } \tilde{\boldsymbol{S}}_{i}, \tilde{\boldsymbol{C}}_{\boldsymbol{S}_{i}}^{e_{k}}(\xi)=0, \tag{25}\\ 0 \leq \text { dist } \boldsymbol{C}_{\boldsymbol{S}_{i}}^{e_{k}}, \tilde{\boldsymbol{C}}_{\boldsymbol{S}_{i}}^{e_{k}}(\xi)<\varepsilon \forall \xi \in\left[\xi_{a}, \xi_{b}\right] \end{gather*}
$$
...
model, according to the definition of a valid B-Rep model. \begin{align*} & O^{\text {intersect }}\left(\boldsymbol{S}_{i}(\boldsymbol{\xi})\right)\left(\tilde{\boldsymbol{S}}_{i}(\boldsymbol{\xi}),\right. \text { where } \\ & \quad \exists(\boldsymbol{\xi}, \boldsymbol{\eta}): \operatorname{dist} \tilde{\boldsymbol{S}}_{i}(\boldsymbol{\xi}), \boldsymbol{S}_{j}(\boldsymbol{\eta})=0 \wedge \operatorname{dist} \tilde{\boldsymbol{S}}_{i}(\boldsymbol{\xi}), \tilde{\boldsymbol{C}}_{k}^{\tilde{\boldsymbol{S}}_{i}}>0 \\ & \quad \forall \tilde{\boldsymbol{C}}_{k}^{\tilde{S}_{i}} \in \Gamma^{\tilde{S}_{i}}, i \neq j \tag{26} \end{align*} with \(\Gamma^{\tilde{\boldsymbol{S}}}{ }_{i}\) being the set of boundary curves of \(\tilde{\boldsymbol{S}}_{i}\).

A special case of intersections are self-intersections:

$$
\begin{equation*} O^{\text {self Intersect }}\left(\boldsymbol{S}_{i}(\boldsymbol{\xi})\right) \mapsto \tilde{\boldsymbol{S}}_{i}(\boldsymbol{\xi}), \text { where } \exists(\boldsymbol{\xi}, \boldsymbol{\eta}): \text { dist } \tilde{\boldsymbol{S}}_{i}(\boldsymbol{\xi}), \boldsymbol{S}_{i}(\boldsymbol{\eta})=0, \boldsymbol{\xi} \neq \boldsymbol{\eta} \tag{27} \end{equation*}
$$

### 2.5 Application of flaw operators

We now continue with the definition of a flawed model. To this end, we apply the flaw operators defined in Section 2.4 onto a valid B-Rep model. The 'dirtiness' of the model is then defined by e. It should be mentioned that, for models that are drafted by a real-life CAD system, flaws do not necessarily originate from these operators, yet most flawed models can equivalently be created by a sequence of these operators.
...
$$
\begin{equation*} \tilde{B}(\tilde{T}, \tilde{G}):=O^{\text {join }} B(T, G), O^{\text {deepCopy }} O^{\text {extract }}\left(B(T, G), t_{i}\right) \tag{28} \end{equation*}
$$
...
$$
\begin{equation*} \tilde{B}(\tilde{T}, \tilde{G}):=O^{\text {delete }} B, f_{i}, g_{f_{i}} \tag{29} \end{equation*}
$$
...
$$
\begin{equation*} \tilde{B}(\tilde{T}, \tilde{G}):=O^{\text {explode }}(B(T, G)) \tag{30} \end{equation*}
$$
...
$$
\begin{equation*} \tilde{B}(\tilde{T}, G):=O^{f l i p}(B(T, G)) \tag{31} \end{equation*}
$$
...
$$
\begin{equation*} \tilde{B}(T, \tilde{G}):=O^{\text {move }}(B(T, G)) \tag{32} \end{equation*}
$$
...
$$
\begin{equation*} \tilde{B}(T, \tilde{G}):=O^{\text {detach }}(B(T, G)) \tag{33} \end{equation*}
$$
...
$$
\begin{equation*} \tilde{B}(T, \tilde{G}):=O^{\text {intersect }}(B(T, G)) \tag{34} \end{equation*}
$$
...
\begin{align*} & \tilde{\omega}^{f_{i}}:=O^{\text {deepCopy }} O^{\text {extract }}\left(B, f_{i}\right) \\ & \breve{\omega}^{f_{i}}:=O^{\text {move }}\left(\boldsymbol{P}_{j} \in\left(\tilde{\omega}^{f_{i}}, \tilde{G}^{t_{i}}\right)\right. \tag{35}\\ & \tilde{B}(\tilde{T}, \tilde{G}):=O^{\text {join }} B, \breve{\omega}^{f_{i}} \end{align*}
...
$$
\begin{align*} \tilde{B}(\tilde{T}, \tilde{G}) & :=O^{\text {explode }}(B) \\ \breve{B}(\tilde{T}, \breve{G}) & :=O^{\text {move }}\left(\tilde{\boldsymbol{P}}_{i} \in \tilde{G}, \tilde{G}\right) \tag{36} \end{align*}
$$
...
![Figure 11. The concept of the Finite Cell Method [56].](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-11-p014.png)

*Figure 11. The concept of the Finite Cell Method [56].: The concept of the Finite Cell Method [56].*

$$
t = 0on aSu a = 0.0 IN Sifict + su=Sphy UNfict → Siphy = 1.0
$$

### 3.1 Basic formulation

In the Finite Cell Method, an n-dimensional open and bounded physical domain \(\Omega_{\text {phy }}\) is embedded in a fictitious domain \(\Omega_{\text {fict }}\) to form an extended domain \(\Omega_{\cup}\), as illustrated in Fig. 11 in two dimensions. The resulting domain \(\Omega\) has a simple shape which can be meshed easily, without conforming to the boundary of \(\Omega_{\text {phy }}\). The weak form of the equilibrium equation for the extended domain Su is defined as

$$
\begin{equation*} \int_{\Omega \cup}[\mathbf{L v}]^{T} \alpha \mathbf{C}[\mathbf{L u}] \mathrm{d} \Omega=\int_{\Omega \cup} \mathbf{v}^{T} \alpha \mathbf{f} \mathrm{~d} \Omega+\int_{\Gamma_{N}} \mathbf{v}^{T} \overline{\mathbf{t}} \mathrm{~d} \Gamma \tag{37} \end{equation*}
$$
...
$$
\alpha(\mathbf{x})=\left\{\begin{array}{ll} 1 & \forall \mathbf{x} \in \Omega_{p h y} \tag{38}\\ 10^{-q} & \forall \mathbf{x} \in \Omega_{f i c t} \end{array},\right.
$$
...
- A flood fill algorithm is applied on TRgeo to mark all connected points as inside and outside, respectively. This yields a filled space tree TRgeo. Remark: For all points that are not on cut leaves, the approximation tree TReo can be used as fast, efficient, and accurate PMC.
...
$$
\begin{equation*} d_{c_{g e o}}>\varepsilon_{g a p} \tag{39} \end{equation*}
$$

\(\varepsilon_{\text {gap }}\) is typically not known apriori and is determined by an iterative decrease of the cell size, until the subsequent fill algorithm fills the entire domain. From this, it follows that the maximal partitioning depth \(n_{\text {max }}\) of \(\widehat{T R}_{\text {geo }}\) is bounded by the ratio of domain size \(d_{d o m a i_{n}}\) of the tree \(\widehat{T R}_{\text {geo }}\) to the dimension of the gaps/openings \(\varepsilon_{g a_{p}}\) :

$$
\begin{equation*} n_{\max }<\log _{2} \frac{d_{\text {domain }}}{\varepsilon_{\text {gap }}} \tag{40} \end{equation*}
$$
...
$$
\begin{equation*} \frac{1}{256}<\frac{\varepsilon_{\text {gap }}}{d_{\text {domain }}}<\frac{1}{128} . \tag{41} \end{equation*}
$$
...
![Figure 12. Example of an STL model with typical flaws.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-12-p017.png)

*Figure 12. Example of an STL model with typical flaws.: Example of an STL model with typical flaws.*

wrongly oriented facet

![Figure 13. Octree approximation of the embedded tetrahedral domain. The outer domain (blue) is separated by the cut leaves (red) from the](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-13-p017.png)
...
![Figure 14. Multiple ray casting for different flaws (red](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-14-p018.png)
...
![Figure 16. Cut through two reconstruction trees for different gap sizes, consequently maximum subdivision depths and for different origin](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-16-p019.png)

*Figure 16. Cut through two reconstruction trees for different gap sizes, consequently maximum subdivision depths and for different origin: Cut through two reconstruction trees for different gap sizes, consequently maximum subdivision depths and for different origin (a)Egap=0.2, max=3, shift byB=0.(b) Egap=0.0031, max = 9, shift by B= 3.*
...
![Figure 17. Relative deviation of energies depending on the gap size for different positions of the cube.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-17-p020.png)
...
![Figure 18. Principal stresses for the flawless model (a) and gap sizes of Egap = 2.5% (b) and Egap = 20% (c).](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-18-p020.png)
...
![Figure 19. Thick-walled plate with circular hole under surface load.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-19-p021.png)
...
![Figure 20. Flawed B-Rep model containing several gaps, intersections, offsets, and multiple entities.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-20-p021.png)
...
![Figure 21. Flaw details](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-21-p021.png)
...
![Figure 22. Approximation tree TRaco with subdivision depth max = 5.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-22-p022.png)
...
![Figure 23. Displacement and finite cell discretization.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-23-p022.png)

*Figure 23. Displacement and finite cell discretization.: Displacement and finite cell discretization.*

![Figure 24. Strain energy norm for polynomial degrees p = 1...6.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-24-p022.png)
...
![Figure 25. Cut through an octree approximation TRgeo, with (a) too small cut leaves (geo (black), so that all (non cut) leaves are marked as](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-25-p023.png)

*Figure 25. Cut through an octree approximation TRgeo, with (a) too small cut leaves (geo (black), so that all (non cut) leaves are marked as: Cut through an octree approximation TRgeo, with (a) too small cut leaves (geo (black), so that all (non cut) leaves are marked as Cut through an octree approximation FRgeo, with (a) too small cut leaves Ceo (black), so that all (non cut leaves are marked as outside (light gray). (b) with one subdivision level less leaves inside (dark gray) can be detected.*

![Figure 26. Finite cell mesh and displacement of the flawed and the valid model.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-26-p023.png)
...
![Figure 27. Von Mises stresses around the flawed region.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-27-p024.png)

*Figure 27. Von Mises stresses around the flawed region.: Von Mises stresses around the flawed region.*

(b) Valid

![Figure 28. General electric design challenge for the optimal shape of a jet engine brake.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-28-p024.png)
...
![Figure 29. Blue lines denote open edges](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-29-p025.png)
...
![Figure 30. Temperature distribution in the bracket due to a point heat source.](/Users/evanthayer/Projects/stepview/docs/2019_integrating_cad_and_numerical_analysis_dirty_geometry_handling_using_the_finite_cell_method/figures/figure-30-p025.png)
...
- C.S. Chong, A. Senthil Kumar, H.P. Lee, Automatic mesh-healing technique for model repair and finite element model generation, Finite Elem. Anal. Des. 43(15)(2007) 1109-1119, http: dx.doi.org 10.1016 j.finel.2007.06.009.
...
- A. Düster, J. Parvizian, Z. Yang, E. Rank, The finite cell method for three-dimensional problems of solid mechanics, Comput. Methods Apple Engro 197(4548) 2008) 3768-3782, http: dx.doi.org 10.1016 j.cma.2008.02.036.
...
- cell method on constructive solid geometry, Comput. Math. Appl. (2017) http: dx.doi.org 10.1016 j.camwa.2017.01.027.

- combined with level set function, Comput. Methods Appl. Mech. Engrg. 278(2014) 361-387, http: dx.doi.org 10.1016 j.cma.2014.06. 007.
...
- M. Joulaian, S. Duczek, U. Gabbert, A. Düster, Finite and spectral cell method for wave propagation in heterogeneous materials, Comput. Mech. 54(3) (2014) 661-675, 661-675,http: dx.doi.org 10.1007 s00466-014-1019-z.
...
- S. Kollmannsberger, A. Özcan, J. Baiges, M. Ruess, E. Rank, A. Reali, Parameterfree, weak imposition of Dirichlet boundary conditions and coupling of trimmed and non-conforming patches, Internat. J. Numer. Methods Engrg. 101(9) (2015) 670-699, http: dx.doi.org 10.1002 nme.4817.
...
- A. Goldberg, D. Robson, Modeling with UML: Language, Concepts, Methods, Springer International Publishing, ISBN: 978-3-319-33932-0, 1983.

- B. Rumpe, Modeling with UML: Language, Concepts, Methods, Springer International Publishing, ISBN: 978-3-319-33932-0, 2016.
...
- (561 A.ds, Ency lopedia of Sombo, That Mechanic, Vol. 2, chestet, est Susex: elmedy, i Sons, iS N: 973-1-119-039-3, Eds.), Encyclopedia of computational Mechanics, 2017, pp. 1-35.
...
- GrabCad, General Electric jet engine bracket challenge-GrabCAD, https: grabcad.com challenges ge-jet-engine-bracket-challenge, 2013.
