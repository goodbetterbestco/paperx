...
![Figure 1.1. Top row](/Users/evanthayer/Projects/stepview/docs/2015_detecting_weakly_simple_polygons/figures/figure-1-1-p002.png)
...
![Figure 2.1. A polygon that many published deﬁnitions incorrectly classify as weakly simple.](/Users/evanthayer/Projects/stepview/docs/2015_detecting_weakly_simple_polygons/figures/figure-2-1-p004.png)
...
![Figure 3.1. Node expansion. Compare with Figure C.1 .](/Users/evanthayer/Projects/stepview/docs/2015_detecting_weakly_simple_polygons/figures/figure-3-1-p005.png)
...
without forks but possibly with spurs, we can is weakly simple.
...
Our algorithm divides each segment expansion into the following four phases: (1) remove all spurs at the endpoints \(u\) and \(v\); (2) compute the nodes \([u a]\) and \([v_{z}]\) in cyclic order around the ellipse \(\partial D_{u_{v}}\); (3) straighten the remaining paths through \(u\) and through \(v\); (4) build the graph \(G_{D}\), check for simple crossings, and update the image graph \(G\). Our analysis uses the following functions of \(u\) (and similar functions of \(v\) ), all defined just before the segment expansion begins:
...
Then for each segment \(v_{z}\) with \(z \neq z^{*}\), we similarly replace subpaths of \(P\) that contain \(v_{z}\) with paths At the end of these two loops, the only remaining subpaths through \(u\) and \(v\) have the forms \(\left[a^{*}, u, v, z^{*}\right]\) or \(\left[a^{*}, u, a^{*}\right]\) or \(\left[z^{*}, v, z^{*}\right]\). To update these subpaths, we intuitively move node \(u\) to \(u^{\prime}\) and move node \(\nu\) to \(\nu^{\prime}\). But in fact, "moving" these two nodes has no effect on our data structures at all; we only change their names for purposes of analysis. The total time for this phase at most \(O((u)\) n (\(u_{0}\)) + (v) n (\(v_{0}\))), where n (\(u_{0}\)) and n (\(v_{0}\)) denote the number of vertices at \(u_{0}\) and \(v_{0}\) after the segment expansion.
...
![Figure A.5. A polygon ( a , b , c , a , b , c , a , x , y , z , x , y , z , x ) that is not weakly simple, even though its rotation number is 1 and every pair of vertices splits the polygon into two paths that do not cross.](/Users/evanthayer/Projects/stepview/docs/2015_detecting_weakly_simple_polygons/figures/figure-a-5-p016.png)
...
![Figure B.1. Subpaths of Q and Q ′ within the strip system of node u .](/Users/evanthayer/Projects/stepview/docs/2015_detecting_weakly_simple_polygons/figures/figure-b-p018.png)
...
Lemma C.1. Let \(P\) be a weakly simple polygon, and let \(D\) be an elliptical disk whose boundary inter sects \(P\) transversely. The polygon \(\tilde{P}\) obtained by expanding \(P\) inside \(D\) is weakly simple.
...
Thus, for any > 0, we obtain a simple polygon ˜ Q such that d \(F(˜P,˜Q)\) < by setting " < ( = 2) sin. We conclude that ˜ P is weakly simple.
...
![Figure C.3. Bar expansion. Compare with Figure 3.1 .](/Users/evanthayer/Projects/stepview/docs/2015_detecting_weakly_simple_polygons/figures/figure-100-3-p021.png)
...
Because Q is simple, the intersection Q ∴ D consists of disjoint simple subpaths. For any two such subpaths and, the endpoints of must lie in one component of @ D ν , and thus the corresponding line segments ˜ and ˜ of ˜ Q are also disjoint. It follows that ˜ Q is also simple. Consider a vertex ˜ p of ˜ P, located at the intersection of an edge pi pi + 1 of P and the ellipse @ D. Let denote the angle between pi pi + 1 and (the line tangent to) @ D at ˜ p. If " is sufficiently small, the corresponding edge qi qi + 1 of Q intersects @ D at a point ˜ q such that \(d(˜p,˜q)<2\) "= sin. It follows that dV ( ˜ P, ˜ Q) < 2 "= sin, where is the minimum angle of intersection between P and @ D.

extracted from the order of forks along each bar, and cyclic order of bars ending at each fork; both of these orders are computed as part of the bar decomposition. Given this sequence of points, which become the new vertices of \(\tilde{P}\), we can perform the rest of the expansion in \(O(m)\) time, where \(m\) is the number of edges of \(P\) that intersect \(D\). If \(P\) is not weakly simple, the expanded polygon \(\tilde{P}\) may include pairs of edges that cross transversely. Let \(G_{D}\) denote the graph whose vertices are the intersection points \(\operatorname{im} P \cap \partial\) and whose edges are distinct edges of \(\tilde{P}\) inside \(D\) and arcs of \(\partial D\) between vertices in cyclic order. \(\tilde{P}\) contains crossing edges if and only if \(G_{D}\) is not a plane graph. We can determine the planarity of \(G_{D}\) as follows. first, add a new "apex" vertex \(a\) and connect it by edges to each of the vertices of \(G_{D}\). The resulting abstract graph \(G_{D}^{+}\)is 3-connected, and therefore has at most one planar embedding. Moreover, \(G_{D}^{+}\)is planar if and only if there is a planar embedding of \(G_{D}\) with all vertices on a single face (which we take to be the outer face) in the correct cyclic order. Thus, \(G_{D}\) is a plane graph if and only if \(G_{D}^{+}\)is a planar graph; there are several linear-time algorithms to determine whether a graph is planar [9,25,31,45]. Crudely, \(G_{D}\) has at most \(O(m)\) vertices and edges, so the planarity check takes at most \(O(m)\) time.

### C.3 Node Expansion

In Section 3, we define node expansion as expansion inside a circular disk \(D_{u}\) of radius \(\delta\) centered at some vertex \(u\) of \(P\), where the radius \(\delta\) is sufficiently small that \(D_{u}\) intersects only edges incident to \(u\).

Lemma C. 2 (Cortese et al. [17, Lemma 2]). Let \(P\) be an arbitrary polygon, and let \(\tilde{P}\) be the result of a single node expansion. \(P\) is weakly simple if and only if \(\tilde{P}\) is weakly simple.

Proof: Suppose \(\tilde{P}\) is weakly simple. Fix a sufficiently small positive real number \(\varepsilon<\delta\). By Theorem 2.1, there is a simple polygon \(\tilde{Q}\) such that \(d_{\mathcal{F}}(\tilde{P}, \tilde{Q}) \leq d_{V}(\tilde{P}, \tilde{Q})<\varepsilon\). We easily observe that \(d_{\mathcal{F}}(P, \tilde{P})<\delta\). Thus, the triangle inequality implies \(d_{\mathcal{F}}(P, \tilde{Q})<d_{\mathcal{F}}(P, \tilde{P})+d_{\mathcal{F}}(\tilde{P}, \tilde{Q})<\delta+\varepsilon<2 \delta\). Because \(\delta\) is arbitrarily small, we conclude that \(P\) is weakly simple. Finally, Lemma C. 1 completes the proof.

### C.4 Bar Expansion

Recall from Section 4 that a bar of \(P\) is a component of the union of all edges of \(P\) that lie on some line. Bar expansion is defined as expansion inside an ellipse \(D_{b}\) defined by a bar \(b\) as follows. Fix a sufficiently small real number \(\delta>0\). Let \(b^{\circ}\) denote the subset of \(b\) containing points at distance at least \(2 \delta\) from the endpoints of \(b\). Finally, \(D_{b}\) is the ellipse whose major axis is \(b^{\circ}\) and whose minor axis has length \(2 \delta\). We emphasize that the endpoints of the bar \(b\) are outside \(D_{b}\).

![Figure C.3. Bar expansion. Compare with Figure 3.1 .](/Users/evanthayer/Projects/stepview/docs/2015_detecting_weakly_simple_polygons/figures/figure-100-3-p021.png)

*Figure C.3. Bar expansion. Compare with Figure 3.1 .: Figure C.3. Bar expansion. Compare with Figure 3.1.*

Lemma C.3. Let \(P\) be a polygon without spurs, and let \(\tilde{P}\) be the result of a single bar expansion. \(P\) is weakly simple if and only if \(\tilde{P}\) is weakly simple.
