...
class MathpixCoordinator:
    def __init__(
        ...
        *,
        ...
        self._thread = Thread(target=self._run, name="mathpix-coordinator", daemon=True)
        self._stop = Event()
    ...
    def _run(self) -> None:
        ...
        with ThreadPoolExecutor(max_workers=self._submit_workers) as executor:
            for paper_id in self._paper_ids:
                ...
                self._publish(
                    paper_id,
                    {"mathpix": {"phase": "submit_queued", "submit_queued_at": self._now_iso()}},
                )
            ...
            while (pending_submissions or submitted_jobs) and not self._stop.is_set():
                ...
                for future in done_submits:
                    ...
                    try:
                        ...
                        self._publish(
                            paper_id,
                            {"mathpix": {"phase": "submitted", "submit_completed_at": self._now_iso(), "pdf_id": pdf_id}},
                        )
                    except Exception as exc:  # pragma: no cover
                        self._publish(
                            paper_id,
                            {"mathpix": {"phase": "submit_failed", "failed_at": self._now_iso(), "error": str(exc)}},
                        )
                        if not paper_future.done():
                            paper_future.set_exception(exc)

                if submitted_jobs and time.monotonic() >= next_poll_at:
                    for paper_id, pdf_id in list(submitted_jobs.items()):
                        ...
                        try:
                            ...
                            self._publish(
                                paper_id,
                                {"mathpix": {"phase": "polling", "last_polled_at": self._now_iso(), "pdf_id": pdf_id, "remote_status": state or "unknown"}},
                            )
                            if state == "completed":
                            ...
                            self._publish(
                                paper_id,
                                {"mathpix": {"phase": "failed", "failed_at": self._now_iso(), "pdf_id": pdf_id, "error": str(exc)}},
                            )
                            if not paper_future.done():
                                paper_future.set_exception(exc)
                            submitted_jobs.pop(paper_id, None)
            ...
            if self._stop.is_set():
                for paper_id, paper_future in self._futures.items():
                    ...
                    self._publish(
                        paper_id,
                        {"mathpix": {"phase": "stopped", "stopped_at": self._now_iso()}},
                    )
                    paper_future.set_exception(RuntimeError(f"Mathpix coordinator stopped before {paper_id} completed."))


__all__ = ["MathpixCoordinator"]
