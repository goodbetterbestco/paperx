...
from pipeline.sources.external import external_layout_path
...
def build_docling_sources(
    ...
    *,
    ...
    docling_json_path = run_docling_impl(paper_id, device=docling_device_impl(), layout=layout)
...
def build_mathpix_sources(
    ...
    *,
    ...
    if not mathpix_credentials_available_impl():
        return None
    mathpix_result = run_mathpix_impl(paper_id, layout=layout)
    return build_mathpix_sources_from_result_impl(paper_id, mathpix_result, layout=layout)
...
def build_extraction_sources_for_paper(
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        docling_future = executor.submit(timed_call_impl, "docling", build_docling_sources_impl, paper_id, layout=layout)
        ...
        if prefetched_mathpix_future is not None:
            ...
            mathpix_sources = build_mathpix_sources_from_result_impl(paper_id, mathpix_result, layout=layout)
