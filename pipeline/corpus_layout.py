...
from pipeline.corpus.paths import (
    ...
    figure_manifest_filename as _figure_manifest_filename,
    paper_uid as _paper_uid,
)
...
class ProjectLayout:
    ...
    def canonical_path(self, paper_id: str) -> Path:
        return self.resolved_data_root() / _canonical_filename(paper_id)
    ...
    def figures_dir(self, paper_id: str) -> Path:
        return self.resolved_figures_root()

    def figure_manifest_path(self, paper_id: str) -> Path:
        return self.figures_dir(paper_id) / _figure_manifest_filename(paper_id)
    ...
    def discover_source_pdfs(self) -> list[Path]:
        root_pdfs = sorted(path for path in self.corpus_root.glob("*.pdf") if path.is_file())
        if root_pdfs:
            raise RuntimeError(
                "Unsupported corpus PDF layout. "
                "Move PDFs into _source/ before running the pipeline."
            )
