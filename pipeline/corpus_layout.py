...
CORPUS_NAME = os.environ.get("PIPELINE_CORPUS_NAME", os.environ.get("PAPER_PIPELINE_CORPUS_NAME", DEFAULT_CORPUS_NAME)).strip() or DEFAULT_CORPUS_NAME
PAPER_ID_TOKEN_RE = re.compile(r"[^a-z0-9]+")
CORPORA_DIR = ROOT / "corpus"


def _configured_project_dir() -> Path | None:
    configured = os.environ.get("PIPELINE_PROJECT_DIR", os.environ.get("PAPER_PIPELINE_PROJECT_DIR", "")).strip()
    if not configured:
        return None
...
def _configured_corpus_dir() -> Path:
    configured = os.environ.get("PIPELINE_CORPUS_DIR", os.environ.get("PAPER_PIPELINE_CORPUS_DIR", "")).strip()
    if configured:
        return Path(configured).expanduser().resolve()
