...
from pipeline.corpus.lexicon import corpus_join_terms
...
def _should_join_fragments(left: str, right: str, *, join_terms: set[str]) -> bool:
...
def _join_fragmented_words(text: str, counts: Counter[str], *, join_terms: set[str]) -> str:
    if zipf_frequency is None:
        return text
    ...
    def replacement(match: re.Match[str]) -> str:
        left = match.group(1)
        right = match.group(2)
        if not _should_join_fragments(left, right, join_terms=join_terms):
            return match.group(0)
...
def normalize_prose_text(
    ...
    *,
    ...
    updated = _join_fragmented_words(updated, counts, join_terms=join_terms)
    updated = _apply_replacements(updated, counts)
