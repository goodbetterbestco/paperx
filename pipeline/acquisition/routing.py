...
class AcquisitionRouteDecision:
    traits: list[str]
    rationale: list[str]
    layout_priority: list[str]
    math_priority: list[str]
...
def route_pdf_signals(signals: AcquisitionSignals) -> AcquisitionRouteDecision:
    ...
    if (
        signals.text_page_ratio < 0.55
        or (signals.avg_text_chars_per_page < 250 and signals.avg_image_coverage >= 0.35)
        or (signals.avg_text_chars_per_page < 500 and signals.avg_image_coverage >= 0.15)
        or signals.two_column_ratio >= 0.4
        or signals.max_text_block_count >= 12
    ):
        layout_priority = ["docling", "mathpix"]
        math_priority = ["mathpix", "docling"]
        rationale.append("layout or text-quality signals indicate a challenging PDF that may need mathpix assistance")
    if not rationale:
