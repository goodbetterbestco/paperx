...
DEFAULT_REMEDIATION_OUTPUT_DIR = ensure_repo_tmp_dir() / "acquisition_remediation"


@dataclass(frozen=True)
class RemediationArtifactPaths:
    output_dir: Path
    history_dir: Path
    summary_json: Path
    snapshot_json: Path
...
def build_remediation_artifact_paths(
    snapshot_label: str,
    *,
    ...
    return RemediationArtifactPaths(
        output_dir=root,
...
def ensure_remediation_artifact_dirs(paths: RemediationArtifactPaths) -> None:
...
def load_current_remediation_summary(*, output_dir: str | Path = DEFAULT_REMEDIATION_OUTPUT_DIR) -> dict[str, Any]:
    path = Path(output_dir) / "summary.json"
    if not path.exists():
        raise FileNotFoundError(f"No remediation summary artifact found at {path.resolve()}")
