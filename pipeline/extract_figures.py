...
from pipeline.corpus_layout import CORPUS_DIR, figure_manifest_path, paper_pdf_path
DOCS_DIR = CORPUS_DIR
...
def _figure_manifest_path(paper_id: str) -> Path:
    return figure_manifest_path(paper_id)


def ensure_figure_manifest(paper_id: str) -> Path:
    manifest_path = _figure_manifest_path(paper_id)
    if manifest_path.exists():
        return manifest_path

    from pipeline.figure_linking import build_manifest_from_pdf_path, process_paper

    pdf_path = paper_pdf_path(paper_id)
    manifest = build_manifest_from_pdf_path(pdf_path.resolve())
    process_paper(manifest)
...
def extract_figures(paper_id: str) -> list[dict[str, Any]]:
    manifest_path = ensure_figure_manifest(paper_id)
