...
from pipeline.output.acquisition_remediation_plan_dashboard_report import (
    render_acquisition_remediation_plan_dashboard_markdown,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Show the acquisition remediation plan execution dashboard.")
    ...
    parser.add_argument("--history-limit", type=int, default=5, help="Number of recent saved plans to include.")
    parser.add_argument("--wave-limit", type=int, default=5, help="Maximum alert rows to highlight per section.")
    parser.add_argument("--format", choices=("json", "markdown"), default="markdown", help="Output format.")
    return parser.parse_args()


def run_show_remediation_plan_dashboard_cli(
    args: argparse.Namespace,
    *,
    summarize_dashboard_fn: Callable[..., dict[str, Any]] = summarize_remediation_plan_dashboard,
    render_markdown_fn: Callable[[dict[str, Any]], str] = render_acquisition_remediation_plan_dashboard_markdown,
...
def main() -> int:
    return run_show_remediation_plan_dashboard_cli(parse_args())
